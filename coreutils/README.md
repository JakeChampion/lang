...
| `tee` | done — `-a` `-i` `-p` `--output-error[=MODE]`, the FILEs opened before the first read and each read block written unbuffered to every output. The mode family is a signal disposition first: without it SIGPIPE keeps its default and tee dies of it, and `-i` / `-p` are SIG_IGN on SIGINT / SIGPIPE (`std/signal`). `tee FILE >&-` diverges until an opened handle stops landing on fd 1 (#8823) |
