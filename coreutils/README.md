...
| `lib/tty.fern` | ttyname(3) as glibc answers it: the `/proc/self/fd/N` readlink first, trusted only when it still stats to the same character device, then a walk of `/dev/pts` and `/dev` by device number. `/dev/ptmx` and `/dev/pts/ptmx` share a device number, which is why the order matters. Shared by `tty` and `logname` |
...
| `lib/procfs.fern` | reading a kernel pseudo-file, which `read_file` cannot do: `/proc` reports `st_size` 0 and `/sys` reports a page, so the content only comes out of a read to EOF. Temporary — it goes when #9065 fixes `read_file` itself |
