...
| `rmdir` | done — `-p` with GNU's textual ancestor walk (trailing slashes off once, then a cut at each last slash stepping back over a run of them), `-v` on stdout before every attempt, `--ignore-fail-on-non-empty` over both the errnos that mean "not empty" and the ones that only might, and the undocumented `--path` synonym that the ambiguity list gives away |
| `readlink` | done — `-f` `-e` `-m` over the canonicalisation walk in `lib/canon.fern`, `-n` (refused for more than one operand, as GNU refuses it), `-q` `-s` `-v` where silence is the default, and `-z`. Needs `getcwd()`, so it is a native-target utility: a WASI component has no working directory |
...
| `lib/canon.fern` | the canonicalisation walk `readlink -f` / `-e` / `-m` share with `realpath` and `ln -r`: `.` dropped, `..` popped off the answer, symlinks spliced, and loops caught by REVISIT rather than by a count |
