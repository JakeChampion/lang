...
| `pathchk` | done — `-p` `-P` `--portability`, and the three different checks they select. The DEFAULT is one `lstat` of the whole name: a path that exists is valid by construction, ENOENT is not a fault, and every other errno IS the diagnostic — so a 256-byte component under an existing directory is `File name too long` while the same name under a missing one is `limit 255 exceeded by length 256 of file name component`. The per-directory `statfs` walk behind that second message runs only for a name that does not exist and only once a component outgrows POSIX's own minimum, which matches GNU's syscall profile call for call. `-p` touches the filesystem not at all (14 and 255, and the portable character set), `-P` is the default PLUS the empty name and a leading `-`, and `-p -P` is neither of them: `-p`'s refusal wins and `-P`'s two rules are added to it. The precedence is the substance — empty, then a dash, then a non-portable byte ANYWHERE in the string, then the whole length, then a component's — and the scan does NOT permute, so `pathchk NAME -p` checks a file called `-p` |
