...
| Free-form    | `test.pass()`, `test.fail(msg)`                                                                                       |
...
```fern
function main(): i32 {
    var r: test.TestRunner = test.test_new("backends");
...
```fern
var arm: test.TestRunner = r.subsuite("arm64");
...
function test_fern_hello(): test.TestOutcome {
    ...
    return test.assert_process(r, 0, "hello");
}
```
...
function main(): i32 {
    var r: test.TestRunner = test.test_new("fuzz");
...
```fern
function main(): i32 {
    var f: string = test.parse_filter_from_args(args());
    var r: test.TestRunner = test.test_new_filtered("my suite", f);
...
function test_modulo_table(): test.TestOutcome {
    ...
    while (i < inputs.len()) {
        if (inputs[i] % 10 != expected[i]) {
            return test.fail("table mismatch at " + i.to_string());
        }
        i = i + 1;
    }
    return test.pass();
