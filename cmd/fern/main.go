...
func main() {
	...
	cover := flag.Bool("cover", false, "measurement build: instrument every executable source line with a hit counter, and write the whole table to stderr at exit — one `fern-cover: <file>:<line> <count>` line per instrumented line, hit or not. Feed the saved stream to `fern -cover-report` for per-file totals, the uncovered-line list, or lcov. Costs an increment per line and keeps functions nothing calls (so they report 0), so it is for measuring a test run, not for shipping; a build without it is byte-identical to one from a compiler without the feature. Native x86-64 and arm64 only — any other target errors rather than emitting an uninstrumented binary. Same as FERN_COVER=1.")
	...
	if *cover {
		// The lowering and both natives read this global directly (the
		// -sanitize precedent), so a late -cover has to be pushed into
		// it. Composes with FERN_COVER=1, which only ever turns it on.
		ast.CoverEnabled = true
		// The instrumentation is a lowering pass, and neither the AST
		// interpreter nor the REPL lowers anything. Accepting -cover
		// there would run the program and print no report — silence a
		// reader would take for "nothing was covered".
		if *doInterp || *repl {
	...
	code, err := run(srcPath, *out, *target, *backend, *emit, *cc, *runIt, *native, *qemu, *componentWrap, *componentWrapCli, *asyncExport, asyncProviders, *shared, *export, *optimize, progArgs)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
...
func run(srcPath, outPath, target, backend, emit, cc string, runIt, native bool, qemu string, componentWrap, componentWrapCli, asyncExport bool, asyncProviders []string, shared bool, export string, optimize bool, progArgs []string) (int, error) {
	...
	if emit == "core-module" {
		...
		if outPath == "" {
			return 1, fmt.Errorf("-emit core-module requires -o OUTPUT")
		}
