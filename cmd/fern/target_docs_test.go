...
// the diagnostic explanations, the header comments of runnable examples, and
// the documentation site's own prose. `docs/` is excluded because it is a
// working record where a line may legitimately quote a target that no longer
// exists.
//
// The site was outside this scope until 2026-09, and shipped `-target
// wasi-http` — a target name that has never existed — on its "Why Fern"
// page for as long as that page had been up.
//
...
func userFacingFiles(t *testing.T, root string) []string {
	...
	skip := map[string]bool{
		...
		filepath.Join(root, "site", "src", "content", "docs", "stdlib"): true,
	}
	var out []string
	add := func(path string, d os.DirEntry, err error) error {
		...
		inSite := strings.Contains(path, filepath.Join("site", "src"))
		switch {
		case d.Name() == "README.md",
			strings.HasSuffix(d.Name(), ".fern"),
			strings.HasSuffix(d.Name(), ".md") && strings.Contains(path, filepath.Join("diag", "explanations")),
			inSite && (strings.HasSuffix(d.Name(), ".md") || strings.HasSuffix(d.Name(), ".mdx") || strings.HasSuffix(d.Name(), ".astro")):
			out = append(out, path)
		}
		return nil
	}
	for _, dir := range []string{
		...
		// The docs site. `src/content/docs/stdlib/` is generated by
		// cmd/ferndoc from stdlib doc comments and is not checked in, so
		// it is skipped: its prose is the .fern sources' prose, already
		// covered above.
