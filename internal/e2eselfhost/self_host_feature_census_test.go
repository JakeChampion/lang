...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "nested named fns", 103,
		"Nested named functions remain in astwalk, checker, parser, irlower and wasm_ir. The capture-type and callable-declaration reconstruction visitors were removed when closure conversion switched to checked capture contracts. The remaining visitors and helper predicates still exercise nested-function compilation, and irlower's per-op lowering gates are one each.")
	pinned(t, c, "try op", 0,
