...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "arrow lambdas", 12,
		"Two are astwalk's no-op statement visitors and one is the checker's, in e060_collect_dyn_locals, which wants fold_stmt_nodes for the statement half and has nothing to say about expressions; three are constfold's assert probe, the first arrow lambdas here that compute rather than return the accumulator untouched. Four are #2673's migration of the anonymous `function` expression — astwalk's splice, checker's diag fold and two parser rewriters — which moved from the row below to this one. They also carry the ANNOTATED return type this row's pattern used to miss, so before that pattern was widened they were counted by no row at all. The last two are the checker's break scans (#8562), loop_can_break and expr_can_break, which fold the value blocks a statement or expression holds and capture the label they look for.")
	pinned(t, c, "anonymous function exprs", 0,
		"Zero since #2673 migrated the last four — astwalk's splice, checker's diag fold and two parser rewriters — to the arrow spelling counted above. All capture, so those plus the capturing nested named fns are still the self-host's only closures; only the spelling moved. This row survives its own subject on purpose: it is what says the migration stays done while the `function` expression form is still parseable. It goes when the form does.")
