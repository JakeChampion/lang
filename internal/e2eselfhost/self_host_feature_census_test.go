...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "nested named fns", 36,
		"Four are astwalk visitors closing over their enclosing function's locals. Seventeen are wasm_ir's helper-gate predicates behind any_op(cache, pred), two of which capture a parameter and the rest capture nothing. The last fifteen are checker collectors riding astwalk: mc_mentions_expr and mc_mentions_stmts take two visitors each (an expression visitor for ExprIdent, a statement visitor for StmtAssign's target, which is a bare string rather than an expression), ow_count_ident takes a tally, e049_expr_lambdas, e049_stmt_own_lambdas, vref_expr and e032_expr each take a visitor plus a descent predicate that prunes at ExprLambda, and e060_e062_stmts and e060_collect_dyn_locals take one visitor each — the first over expressions closing over the dyn name set, the second over statements, since a `var`'s name and annotation are fields on the statement.")
	pinned(t, c, "try op", 0,
	...
	atLeast(t, c, "for..in loops", 307,
		"292 in checker.fern and 15 in visibility.fern. A fall USUALLY means a module went back to hand-indexing — but not always: folding a collector onto astwalk removes its for..in loops along with its variant arms. That has now lowered this floor five times running (326 -> 325 for mc_mentions, 325 -> 321 for e049_expr_lambdas, 321 -> 317 for vref_expr, 317 -> 313 for e032_expr, 313 -> 307 for the e060/e062 pair), which is worth noticing: while the SH-022 walker migration runs, this row and `astwalk call sites` move in OPPOSITE directions, so a floor here has to be lowered on every conversion and protects nothing in between. The astwalk floor is the one carrying the signal for that work. Lower this one for a conversion; investigate it otherwise.")
	atLeast(t, c, "astwalk call sites", 98,
