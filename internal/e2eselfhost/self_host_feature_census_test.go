...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "nested named fns", 69,
		"Sixteen are parser's: the mentions visitors (expr_mentions / stmt_mentions) and the fn-value-call visitors (fnv_name_called_in_expr / _stmt), each closing over the name being sought, the moves-handle pair in stmts_move_handle — an expression visitor closing over the function table and the name, and a statement visitor for StmtAssign's target — the deep defer scan's pair in dl_stmts_have_defer_deep (a StmtDefer probe plus a no-op expression visitor), the elb guard pairs in elb_assigns_stmt and elb_idx_monotonic_stmts, each a statement visitor closing over the guarded name plus a no-op expression visitor, and the cf const-rule family (the two scanners' expression+statement visitors and the two rewrite closures, eight in all, each closing over the name being folded), the lambda-lift's two capture-threading rewrite closures, and the hl pairs in hl_collect_var_names and hl_has_rec_local — a var-name collector and a recursive-local probe, each with a no-op expression visitor — the first consumers in parser.fern to ride the spine (#6993). Four are astwalk visitors closing over their enclosing function's locals. Seventeen are wasm_ir's helper-gate predicates behind any_op(cache, pred), two of which capture a parameter and the rest capture nothing. The last sixteen are checker collectors riding astwalk: mc_mentions_expr and mc_mentions_stmts take two visitors each (an expression visitor for ExprIdent, a statement visitor for StmtAssign's target, which is a bare string rather than an expression), ow_count_ident takes a tally, e049_expr_lambdas, e049_stmt_own_lambdas, vref_expr and e032_expr each take a visitor plus a descent predicate that prunes at ExprLambda, e060_e062_stmts and e060_collect_dyn_locals take one visitor each — the first over expressions closing over the dyn name set, the second over statements, since a `var`'s name and annotation are fields on the statement, e053_own_exprs wraps e053_at_node so the visitor closes over PARAMETERS rather than the driver loop's per-statement `cur`, and match_stmt_own_lambdas, dupvar_stmt_own_lambdas and assign_stmt_own_lambdas each take a visitor plus a prune predicate to reach the statements inside a lambda body without re-reporting nested ones — the last of those builds the body's SCOPE (params plus return type) before recursing, since its pass is scope-threaded.")
	pinned(t, c, "try op", 0,
	...
	atLeast(t, c, "astwalk call sites", 136,
