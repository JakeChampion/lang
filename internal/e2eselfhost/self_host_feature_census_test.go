...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	atLeast(t, c, "for..in loops", 313,
		"298 in checker.fern and 15 in visibility.fern. A fall USUALLY means a module went back to hand-indexing — but not always: folding a collector onto astwalk removes its for..in loops along with its variant arms. That has now lowered this floor four times running (326 -> 325 for mc_mentions, 325 -> 321 for e049_expr_lambdas, 321 -> 317 for vref_expr, 317 -> 313 for e032_expr), which is worth noticing: while the SH-022 walker migration runs, this row and `astwalk call sites` move in OPPOSITE directions, so a floor here has to be lowered on every conversion and protects nothing in between. The astwalk floor is the one carrying the signal for that work. Lower this one for a conversion; investigate it otherwise.")
