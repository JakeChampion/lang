...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "nested named fns", 106,
		"Nested named functions remain in astwalk, checker, parser, irlower and wasm_ir. The capture-type and callable-declaration reconstruction visitors were removed when closure conversion switched to checked capture contracts. The remaining visitors and helper predicates still exercise nested-function compilation. The 103 -> 105 step is #9203's counted_handback_names in irlower.fern: the two visitors (at_stmt, at_expr) it hands astwalk.fold_stmt_nodes, closing over the body, params, struct table and handback rows the walk needs. The 105 -> 106 step is #9128's is_crc32 in wasm_ir.fern, the op predicate module_uses_crc32_cksum hands any_op — the same nested-predicate shape its five byte-kernel siblings directly above it already carry.")
	pinned(t, c, "try op", 0,
