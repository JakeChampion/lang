...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "arrow lambdas", 12,
		"Two are astwalk's no-op statement visitors and one is the checker's, in e060_collect_dyn_locals, which wants fold_stmt_nodes for the statement half and has nothing to say about expressions; three are constfold's assert probe, the first arrow lambdas here that compute rather than return the accumulator untouched. The other six came from #2673's migration of the anonymous `function` expression — astwalk's splice, checker's diag fold and two parser rewriters, plus two more checker folds that landed while it was in flight. All capture, so these plus the capturing nested named fns are the self-host's only closures. The `anonymous function exprs` row this replaces is gone with its subject: the parser rejects that spelling now (P006), so nothing needs to count it.")
	pinned(t, c, "nested named fns", 105,
		"Thirty-one are irlower's, riding astwalk — the cap-family's cap_type_of, init_of and fn_ret_of, body_binds_lambda's binds_lambda_at, and the closure-array family's four (the credit's cand_site_of and binds_fresh_at, and the empty-`fn[]`-declaration scan's box_appended_names and empty_fnarr_decl, #4354) on the statement spine, each closing over the name or body being resolved, the five visit/descend wrapper pairs of the Perceus escape-scanner trio (rctuple_esc_expr, arrtup_elem_esc_expr + its payload driver, arrstruct_elem_esc_expr + its payload driver), each a two-line closure handing the family's context to its top-level shape classifier and visitor (#6993; the assign-targets family's visitors capture nothing and are plain top-level functions), plus the env-box lift's nine on the accumulator spine: lift_inline_closures_expr, lift_inline_closures and lift_iife_conditions each carry a statement/expression wrapper pair handing fd/gfns/structs/mfuncs to ilc_stmt_at / ilc_expr_at, and box_iife_arm_values, box_iife_arm_array_elems and lift_iife_arm_values each carry one statement visitor over the shared top-level claim-all ilc_keep_expr, and the superseded-field own move's three (#8186): field_move_bases_of's bad_of, closing over the signature registry and the bound set, and the fold_expr tallies of count_ident_reads and count_field_reads, each closing over the name being counted — its capture-free scans (defer_at, field_move_decl_of, lambda_body_reads) are top-level, over the shared no-op visitors no_expr_bool, no_expr_strs and no_stmt_strs. The thirty-first is the own-lift scan's pick (#8267), nested in own_lift_stmt_sites and closing over the argument accumulator; own_lift_arg_sites_of and own_lift_scan_uses beside it are capture-free and top-level. Twelve are parser's: the mentions visitors (expr_mentions / stmt_mentions) and the fn-value-call visitors (fnv_name_called_in_expr / _stmt), each closing over the name being sought, the moves-handle pair in stmts_move_handle — an expression visitor closing over the function table and the name, and a statement visitor for StmtAssign's target — the deep defer scan's pair in dl_stmts_have_defer_deep (a StmtDefer probe plus a no-op expression visitor), the elb guards elb_assigns_stmt and elb_idx_monotonic_stmts, each a statement visitor closing over the guarded name, and the cf const-rule family (the two scanners' expression+statement visitors and the two rewrite closures, eight in all, each closing over the name being folded), the lambda-lift's two capture-threading rewrite closures, the named/default-argument pass's two rewrite closures, and hl_collect_var_names and hl_has_rec_local — a var-name collector and a recursive-local probe — the first consumers in parser.fern to ride the spine (#6993). Those four lost their paired no-op expression visitors to astwalk.fold_stmt_spine (#8180); dl_stmts_have_defer_deep keeps its pair because it descends into expressions. Four are astwalk visitors closing over their enclosing function's locals. Twenty are wasm_ir's helper-gate predicates behind any_op(cache, pred), two of which capture a parameter and the rest capture nothing. The last seventeen are checker collectors riding astwalk: mc_mentions_expr and mc_mentions_stmts take two visitors each (an expression visitor for ExprIdent, a statement visitor for StmtAssign's target, which is a bare string rather than an expression), ow_count_ident takes a tally, as does ow_count_field_reads (#8186), e049_expr_lambdas, e049_stmt_own_lambdas, vref_expr and e032_expr each take a visitor plus a descent predicate that prunes at ExprLambda, e060_e062_stmts and e060_collect_dyn_locals take one visitor each — the first over expressions closing over the dyn name set, the second over statements, since a `var`'s name and annotation are fields on the statement, e053_own_exprs wraps e053_at_node so the visitor closes over PARAMETERS rather than the driver loop's per-statement `cur`, and match_stmt_own_lambdas, dupvar_stmt_own_lambdas and assign_stmt_own_lambdas each take a visitor plus a prune predicate to reach the statements inside a lambda body without re-reporting nested ones — the last of those builds the body's SCOPE (params plus return type) before recursing, since its pass is scope-threaded. The 101 -> 103 step is #8639's suffixed-literal range rule in checker.fern: at_unary and at_number, the two astwalk visitors the rule rides — at_unary collecting the sign chain a suffixed literal sits under (so -9223372036854775808i64 is judged in range and 9223372036854775808i64 is not), and at_number reporting E047 at the literal itself. Both close over the accumulator being folded, which is what makes them nested rather than top-level. The 103 -> 104 step is #8657's value-block local retype in checker.fern: vb_expr's `at`, the map_expr visitor closing over the SCOPE the rewritten block sits in — the rewrite has to type what the arms store, and the spine hands its visitor the node alone. Its pre-scan visitor (vb_at_value_local) captures nothing and is top-level. The 104 -> 105 step is #8791's __mismatch: one more helper-gate predicate in wasm_ir, capture-free like the rest of that family.")
	pinned(t, c, "try op", 0,
	...
	pinned(t, c, "Map type spellings", 12,
		"irverify's NameIndex, wasm_ir's call set, and builtins' mirror of std/json's JObject payload. The 11 -> 12 step is #8611's linear-space diff in printer.fern: the id table that interns both sides' lines so the alignment compares i32 keys instead of strings, which is what lets the Hirschberg pass hold two rows rather than the O(m*n) table it replaces.")
