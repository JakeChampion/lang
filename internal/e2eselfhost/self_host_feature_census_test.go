...
func TestSelfHostFeatureCensus(t *testing.T) {
	...
	pinned(t, c, "arrow lambdas", 15,
		"The shared AST splicer now has an identity lambda finish hook. Its other arrow lambdas are traversal callbacks in astwalk, checker, parser and constfold. The 13 -> 14 step is the checker's value-block return walk (ret_diags_in_exprs, #9828), a fold_expr_pruned callback closing over the enclosing function's return type, spelled as its loop_diags_in_exprs sibling is. The 14 -> 15 step is the AST lowering's cell-read desugar (cell_reads_as_index, #9320), a map_stmts visitor closing over the cell names. The 15 -> 14 step deletes the parser's call-only fn-value inlining (#10106), whose descent carried one. The 14 -> 15 step is the checker's unannotated-lambda exit walk (lambda_try_exits, #9518), a fold_expr_pruned callback closing over the statement's scope to type each `?` operand. These sites exercise closure compilation on the self-host path.")
	pinned(t, c, "nested named fns", 105,
		"Nested named functions remain in astwalk, checker, parser, irlower and wasm_ir. The 106 -> 109 change adds three scalar identifier-count visitors in astwalk: count_ident_expr's expression visitor and count_ident_stmts' statement and expression visitors. Each captures the queried name and folds an integer instead of building a full identifier list. Identifier-count oracle tests preserve the existing assignment-target and lambda-scope rules. The 109 -> 110 step is wasm_ir's module_uses_scale_f64 predicate for the eighth kernel, spelled as its module_uses_sum_bytes sibling is. The 110 -> 111 step is module_uses_scan_set, the byte-set scan kernel's predicate, spelled the same way, 111 -> 112 is module_uses_count_runs, the run-count kernel's, and 112 -> 113 module_uses_bsd_sum, the BSD checksum's. The 113 -> 110 step deletes irlower's function-array promotion scans (#10076), which carried three nested visitors, and 110 -> 102 deletes the parser's call-only fn-value inlining (#10106), whose eight use-count scanners were nested visitors. The 102 -> 104 step is parser's retag_type_arg_indexes (#7040): a map_stmts visitor per function and one for the top-level statements, each closing over the generic-function and type names that decide whether `f[Box](x)` carries a type argument. The 104 -> 105 step is the checker's enclosing-scope E049 (#8440): e049_var_init_is_lambda's statement visitor, closing over the variable name it looks for.")
	pinned(t, c, "try op", 0,
	...
	pinned(t, c, "Map type spellings", 8,
