...
// TestSelfHostExternBoolRecordFieldCustomProvider is the self-host port of the
// bool record field (docs/WIT-BRING-YOUR-OWN.md, Go-side
// TestExternBoolRecordFieldCustomProvider). A WIT bool flattens to a single i32
// (param) and is one byte canonically (result); a self-host bool field is 0/1 in
// a 4-byte slot, so it's handled as an unsigned 8-bit (extern_field_load_op →
// i32.load8_u, extern_canon_field_size → 1). Verifies both directions round-trip.
//
// mk: func(n: s32) -> flag returns { on: (n odd), n } (bool field in a result);
// rd: func(f: flag) -> s32 returns f.on ? f.n : -f.n (bool field in a param).
func TestSelfHostExternBoolRecordFieldCustomProvider(t *testing.T) {
	...
	const iface = "interface sink { record flag { on: bool, n: s32 } mk: func(n: s32) -> flag; rd: func(f: flag) -> s32; }"
	...
	// mk returns the record indirectly (on:bool@0, n:s32@4) via cabi_realloc,
	// on = n&1. rd receives the record flattened to (on:i32, n:i32).
  ...
  (func (export "cabi_realloc") (param $op i32) (param $os i32) (param $al i32) (param $ns i32) (result i32)
    ...
    (local.get $p))
  (func (export "local:test/sink@0.1.0#mk") (param $n i32) (result i32)
    (local $r i32)
    (local.set $r (call 0 (i32.const 0) (i32.const 0) (i32.const 4) (i32.const 8)))
    (i32.store8 (local.get $r) (i32.and (local.get $n) (i32.const 1)))
    (i32.store offset=4 (local.get $r) (local.get $n))
    (local.get $r))
  (func (export "local:test/sink@0.1.0#rd") (param $on i32) (param $n i32) (result i32)
    (if (result i32) (local.get $on) (then (local.get $n)) (else (i32.sub (i32.const 0) (local.get $n))))))`), 0o644); err != nil {
...
`
	if err := os.WriteFile(filepath.Join(dir, "bf_run.fern"), []byte(driver), 0o644); err != nil {
		t.Fatalf("write driver: %v", err)
	}
	driverBin := buildSelfHostBin(t, gcc, dir, "bf_run.fern", "bf_run")

	const want = "flag-ok"
	prog := `struct Flag { on: boolean, n: i32 }
@import("local:test/sink@0.1.0", "mk")
function mk(n: i32): Flag;
@import("local:test/sink@0.1.0", "rd")
