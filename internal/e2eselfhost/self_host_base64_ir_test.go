...
var base64IRCases = []struct {
	...
	{"encode-len-pad", `return base64_encode("hi").len();`},                  // "aGk=" -> 4
	{"encode-len-exact", `return base64_encode("Man").len();`},               // "TWFu" -> 4
	{"encode-digit", `return base64_encode("Man")[0] as i32;`},               // 'T' = 84
	{"decode-len", `return base64_decode("aGk=").len();`},                    // "hi" -> 2
	{"roundtrip", `return base64_decode(base64_encode("Hi"))[0] as i32;`},    // 'H' = 72
	{"roundtrip-len", `return base64_decode(base64_encode("hello")).len();`}, // 5
}
