...
// the live variant's payloads, in BOTH `__struct_drop_<T>` and
// `__field_reclaim_<T>`.
//
// `k_enum` in struct_drop_field_ops and `fr_enum` in field_reclaim_field_ops
// each fell through their arm chain to the same shallow `arr_dec`. Neither had a
// walk to reach for: `__enum_arr_elems_drop_<E>` is a variant walk and all three
// backends emit it, but it takes a BUFFER and loops. What was missing was the
// single-box step.
//
// So the per-element body inside that loop is now its own helper,
...
//
// A third gate had to move with it. `nested_field_deep_drop_ok` decides whether a
// struct is worth deep-dropping at all, and it counted only arrays and strings as
// reclaimable — so `Inner { p: Payload }` looked like it owned nothing and never
// got a `__struct_drop_Inner` emitted. An admitted enum field now counts, which is
// what makes the struct_drop route below reachable.
//
...
// three backends emit, and where an over-release from the added walk would land
// as exit 99.
var selfHostEnumFieldDeepDropCases = []struct {
	...
	// Route 2: one level down, so `__struct_drop_Inner` is what releases it. This
	// row is also what proves the nested_field_deep_drop_ok change — without it no
	// __struct_drop_Inner is emitted at all.
...
//
// SIX rows leak without the fix: the four defect rows and BOTH guard rows. The
// guards earn their name from the other direction — they are the rows an
...
// asserted alongside the verdict. Only the two controls are clean either way.
//
