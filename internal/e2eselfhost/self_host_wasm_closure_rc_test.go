...
func TestSelfHostRcClosureWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"clos-box-freed", "function main(): i32 { var n: i32 = 5; var f = (x: i32): i32 => { return x + n; }; return f(37) + __rc_underflow_count(); }", 42},
		// multi-capture scalar closure freed; detector 0.
		{"clos-multi", "function main(): i32 { var a: i32 = 30; var b: i32 = 12; var f = (): i32 => { return a + b; }; return f() + __rc_underflow_count(); }", 42},
		// string capture: box freed (capture leaks one level — sound), value ok.
		{"clos-string-cap", "function main(): i32 { var s: string = \"hello\"; var f = (): i32 => { return s.len(); }; return f() + 37 + __rc_underflow_count(); }", 42},
		// churn: 200k scalar-capture closures built + dropped; boxes reclaim
		// each cycle (no OOM), detector 0 — proves real box free.
		{"clos-scalar-churn", "function mk(k: i32): i32 { var n: i32 = k; var f = (x: i32): i32 => { return x + n; }; return f(1); } function main(): i32 { var s = 0; var k = 0; while (k < 200000) { s = mk(k); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 3},
		// returned closure (move-on-return) excluded from sweep; caller's
		// binding (call init) not swept — no double free, value-correct.
		{"clos-return", "function adder(a: i32): (i32) => i32 { var f = (b: i32): i32 => { return a + b; }; return f; } function main(): i32 { var add10 = adder(10); var add20 = adder(20); return add10(5) + add20(7) + __rc_underflow_count(); }", 42},
		...
		{"cap-string-released", "function main(): i32 { var s: string = \"ab\" + \"cd\"; var f = (): i32 => { return s.len(); }; return f() + 38 + __rc_underflow_count(); }", 42},
		...
		{"cap-string-churn", "function mk(): i32 { var s: string = \"ab\" + \"cd\"; var f = (): i32 => { return s.len(); }; return f(); } function main(): i32 { var k = 0; var n = 0; while (k < 200000) { n = mk(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 4},
		// Slice 2: an ARRAY capture (i32[]) reclaims each cycle across 200k.
		{"cap-array-churn", "function mk(): i32 { var xs: i32[] = [10, 20, 30]; var f = (): i32 => { return xs[1]; }; return f(); } function main(): i32 { var k = 0; var n = 0; while (k < 200000) { n = mk(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 6},
		...
		{"cap-struct-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var i = Inner { xs: [1, 2, 3, 4], n: 5 }; var f = (): i32 => { return i.n; }; return f(); } function main(): i32 { var k = 0; var n = 0; while (k < 200000) { n = mk(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 5},
		...
		{"cap-mixed", "function main(): i32 { var s: string = \"xy\" + \"zw\"; var a: i32 = 38; var f = (): i32 => { return s.len() + a; }; return f() + __rc_underflow_count(); }", 42},
		...
		{"cap-structarr-val", "struct Inner { xs: i32[], n: i32 } function main(): i32 { var ps: Inner[] = [Inner { xs: [1, 2], n: 40 }, Inner { xs: [3, 4], n: 9 }]; var f = (): i32 => { return ps[0].n + ps[1].xs[1]; }; return f() + __rc_underflow_count(); }", 44},
		...
		{"cap-structarr-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var ps: Inner[] = [Inner { xs: [1, 2, 3], n: 4 }, Inner { xs: [5, 6, 7], n: 8 }]; var f = (): i32 => { return ps[0].n; }; return f(); } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 4},
	}
