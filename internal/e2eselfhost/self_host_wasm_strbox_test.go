...
func TestSelfHostRcStrBoxWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"concat-alias-clean", "function main(): i32 { var s: string = \"ab\" + \"cd\"; var t: string = s; return t.len() + __rc_underflow_count(); }", 4},
		...
		{"string-struct-survives-free", "struct H { name: string } function mk(): H { var s: string = \"xy\" + \"z\"; return H { name: s }; } function main(): i32 { var h = mk(); return h.name.len() + __rc_underflow_count(); }", 3},
		...
		{"string-build-use-churn", "function main(): i32 { var n = 0; var k = 0; while (k < 2000) { var s: string = \"ab\" + \"cd\"; n = n + s.len(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 6},
		...
		{"string-builder-loop-reclaim", "function main(): i32 { var s: string = \"\"; var i = 0; while (i < 100000) { s = s + \"x\"; i = i + 1; } return (s.len() % 7) + __rc_underflow_count(); }", 5},
		// Rebind reclaim: `var s = …` re-bound each iteration is released
		// per-iteration (StmtVar dec-on-overwrite). detector clean.
		{"string-rebind-loop-reclaim", "function main(): i32 { var n = 0; var k = 0; while (k < 100000) { var s: string = \"ab\" + \"cd\"; n = n + s.len(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 6},
