...
var lambdaStructFieldIRCases = []struct {
	...
	{"one-field", `struct Box { f: (i32) => i32 } function main(): i32 { var b = Box { f: (x: i32): i32 => { return x + 1; } }; return (b.f)(10); }`, 11},
	{"two-fields", `struct Ops { inc: (i32) => i32, dbl: (i32) => i32 } function main(): i32 { var o = Ops { inc: (x: i32): i32 => { return x + 1; }, dbl: (x: i32): i32 => { return x * 2; } }; return (o.inc)(10) + (o.dbl)(10); }`, 31},
	{"mixed-scalar-and-fn", `struct Wrap { n: i32, f: (i32) => i32 } function main(): i32 { var w = Wrap { n: 5, f: (x: i32): i32 => { return x * 3; } }; return w.n + (w.f)(10); }`, 35},
	// #3445 case 1: a CAPTURING lambda in a struct field (captures the local `n`).
	{"capturing-field", `struct Box { f: (i32) => i32 } function main(): i32 { var n = 10; var b = Box { f: (x: i32): i32 => { return x + n; } }; return (b.f)(7); }`, 17},
	// Two captured locals in the struct-field closure.
	{"capturing-two-caps", `struct Box { f: (i32) => i32 } function main(): i32 { var a = 8; var b = 100; var bx = Box { f: (x: i32): i32 => { return x + a + b; } }; return (bx.f)(0); }`, 108},
	// A capturing closure field alongside a scalar field (the env box and the
	// i32 field coexist in the struct).
	{"capturing-mixed-field", `struct Wrap { n: i32, f: (i32) => i32 } function main(): i32 { var k = 3; var w = Wrap { n: 5, f: (x: i32): i32 => { return x * k; } }; return w.n + (w.f)(10); }`, 35},
}
