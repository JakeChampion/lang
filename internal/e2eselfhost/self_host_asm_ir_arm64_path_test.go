...
func TestSelfHostAsmIRArm64Path(t *testing.T) {
	...
	irOnly := []struct {
		...
		{"read-file-bytes", `function main(): i32 { var p: string = "/tmp/fern_rfb_a64.bin"; match (write_file(p, "AZ")) { Ok(_) => {}, Err(_) => { return 1; } } match (read_file_bytes(p)) { Ok(b) => { if (b.len() != 2) { return 2; } if ((b[0] as i32) != 65) { return 3; } if ((b[1] as i32) != 90) { return 4; } }, Err(_) => { return 5; } } match (remove_file(p)) { Ok(_) => {}, Err(_) => { return 6; } } match (read_file_bytes(p)) { Ok(_) => { return 7; }, Err(e) => { match (e) { NotFound(_) => {}, _ => { return 8; } } } } return 45; }`, 45},
		{"read-file-invalid-utf8", `function main(): i32 { var p: string = "/tmp/fern_rfu_a64.bin"; var raw: u8[] = [255 as u8, 254 as u8]; match (write_file(p, string_from_bytes_unchecked(raw))) { Ok(_) => {}, Err(_) => { return 1; } } match (read_file(p)) { Ok(_) => { return 2; }, Err(e) => { match (e) { InvalidUtf8(q) => { if (q != p) { return 3; } }, _ => { return 4; } } } } match (read_file_bytes(p)) { Ok(b) => { if (b.len() != 2) { return 5; } if ((b[0] as i32) != 255) { return 6; } }, Err(_) => { return 7; } } var _r: Result[void, IoError] = remove_file(p); return 46; }`, 46},
