...
func TestSelfHostTcpClientIRArm64(t *testing.T) {
	...
	// tcp_close and tcp_connect are migrated (#2649) and so carry the
	// stack-ABI __fn_ prefix; send/recv/pollable are still hand-asm.
	for _, sym := range []string{"bl __fn___fern_tcp_connect", "bl __fern_tcp_send", "bl __fern_tcp_recv", "bl __fn___fern_tcp_close", "bl __fern_tcp_pollable"} {
