...
func TestSelfHostRcOptionBoxWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"result-ok-intact", "function getr(b: i32): Result[i32, i32] { if (b > 0) { return Ok(7); } return Err(9); } function main(): i32 { match (getr(1)) { Ok(x) => { return x + __rc_underflow_count(); }, Err(e) => { return 0; } } }", 7},
		...
		{"option-swept-clean", "function main(): i32 { var o = Some(40); match (o) { Some(x) => { return x + 2 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		// Aliasing an option: the alias is inc'd, both swept, balanced.
		{"option-alias-clean", "function main(): i32 { var o = Some(20); var u = o; match (u) { Some(x) => { return x + 22 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		// A Some holding a heap string, swept (counting) — detector clean.
		{"option-string-payload-swept", "function main(): i32 { var s: string = \"ab\" + \"cd\"; var o = Some(s); match (o) { Some(v) => { return v.len() + 38 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		...
		{"option-move-return-clean", "function mk(b: i32): Option[i32] { if (b > 0) { return Some(33); } return None; } function main(): i32 { var o = mk(1); var p = mk(0); match (o) { Some(x) => { return x + 9 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		// An option re-bound each loop iteration: detector stays clean (counting;
		// free off, so intermediates leak soundly).
		{"option-loop-clean", "function main(): i32 { var s = 0; var k = 0; while (k < 1000) { var o = Some(k); match (o) { Some(x) => { s = s + 2; }, None => {} } k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 5},
		...
		{"option-readfile-result-swept", "function main(): i32 { var r = read_file(\"definitely_missing_xyz.txt\"); match (r) { Ok(s) => { return s.len(); }, Err(e) => { return 42 + __rc_underflow_count(); } } }", 42},
		...
		{"option-string-payload-released", "function main(): i32 { var s: string = \"ab\" + \"cd\"; var o = Some(s); match (o) { Some(v) => { return v.len() + 38 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		...
		{"option-i32-payload-flat-safe", "function main(): i32 { var o = Some(262184); match (o) { Some(x) => { return 42 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		...
		{"option-builder-escape-clean", "function mk(): Option[string] { var s: string = \"x\" + \"yz\"; return Some(s); } function main(): i32 { var o = mk(); var p = mk(); match (o) { Some(v) => { return v.len() + 39 + __rc_underflow_count(); }, None => { return 0; } } }", 42},
		// A churn of Some(heap string): payload release reclaims the strings (no
		// growth), detector clean across many cycles.
		{"option-string-churn-clean", "function mk(): i32 { var o = Some(\"a\" + \"b\"); match (o) { Some(v) => { return v.len(); }, None => { return 0; } } } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 2},
		...
		{"option-rebind-loop-reclaim", "function main(): i32 { var n = 0; var k = 0; while (k < 100000) { var o = Some(\"a\" + \"b\"); match (o) { Some(v) => { n = n + v.len(); }, None => {} } k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 3},
		...
		{"option-reassign-loop-reclaim", "function main(): i32 { var o = Some(\"xx\" + \"yy\"); var k = 0; while (k < 100000) { o = Some(\"z\" + \"w\"); k = k + 1; } match (o) { Some(v) => { return v.len() + __rc_underflow_count(); }, None => { return 0; } } }", 2},
		...
		{"result-err-string-released", "function fail(): Result[i32, string] { return Err(\"e\" + \"rr\"); } function main(): i32 { var r: Result[i32, string] = fail(); match (r) { Ok(x) => { return x; }, Err(e) => { return e.len() + 39 + __rc_underflow_count(); } } }", 42},
		// The Ok side still releases (Ok string payload), Err type scalar.
		{"result-ok-string-released", "function good(): Result[string, i32] { return Ok(\"o\" + \"k!\"); } function main(): i32 { var r: Result[string, i32] = good(); match (r) { Ok(s) => { return s.len() + 39 + __rc_underflow_count(); }, Err(e) => { return e; } } }", 42},
		// A churn building + freeing an Err(heap-string) 50k times: the Err
		// string is reclaimed each cycle (no growth), detector clean.
		{"result-err-string-churn-clean", "function fail(): Result[i32, string] { return Err(\"a\" + \"b\"); } function mk(): i32 { var r: Result[i32, string] = fail(); match (r) { Ok(x) => { return x; }, Err(e) => { return e.len(); } } } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 2},
		...
		{"option-arr-unannotated-freed", `function main(): i32 { var o = Some([1, 2, 3]); var r = 0; match (o) { Some(v) => { r = v[0] + v[1] + v[2] + 36; }, None => {} } return r + __rc_underflow_count(); }`, 42},
		...
		{"option-arr-payload-churn-clean", `function mk(): i32 { var o: Option[i32[]] = Some([1, 2, 3]); var r = 0; match (o) { Some(v) => { r = v.len(); }, None => {} } return r; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }`, 3},
		// Result Ok([..]) array payload freed the same way on wasm (tag 0).
		{"result-ok-arr-payload-freed", `function main(): i32 { var r: Result[i32[], i32] = Ok([5, 6, 7]); var x = 0; match (r) { Ok(v) => { x = v[0] + v[2] + 30; }, Err(e) => { x = e; } } return x + __rc_underflow_count(); }`, 42},
		...
		{"option-struct-payload-freed", `struct P { x: i32, y: i32 } function main(): i32 { var o: Option[P] = Some(P { x: 18, y: 24 }); var r = 0; match (o) { Some(p) => { r = p.x + p.y; }, None => {} } return r + __rc_underflow_count(); }`, 42},
		...
		// + detector clean.
		{"option-struct-arrfield-payload-freed", `struct Buf { xs: i32[], n: i32 } function main(): i32 { var o: Option[Buf] = Some(Buf { xs: [10, 20, 30], n: 9 }); var r = 0; match (o) { Some(b) => { r = b.xs[1] + b.n; }, None => {} } return r + 13 + __rc_underflow_count(); }`, 42},
		...
		// drives wasm too).
		{"option-precise-if-freed", `function f(n: i32): i32 { var o: Option[i32] = Some(40); var c = 0; if (n > 0) { match (o) { Some(v) => { c = v; }, None => {} } } return c + 2 + __rc_underflow_count(); } function main(): i32 { return f(5); }`, 42},
		...
		// detector clean.
		{"option-arr-precise-if-freed", `function f(n: i32): i32 { var o: Option[i32[]] = Some([10, 20, 30]); var c = 0; if (n > 0) { match (o) { Some(v) => { c = v[0] + v[2]; }, None => {} } } return c + 2 + __rc_underflow_count(); } function main(): i32 { return f(5); }`, 42},
		...
		{"result-scalar-precise-if-freed", `function f(n: i32): i32 { var r: Result[i32, i32] = Ok(40); var c = 0; if (n > 0) { match (r) { Ok(v) => { c = v; }, Err(e) => { c = e; } } } return c + 2 + __rc_underflow_count(); } function main(): i32 { return f(5); }`, 42},
		// PRECISE drop of an rc-PAYLOAD (array) RESULT Ok box in a nested if.
		{"result-arr-precise-if-freed", `function f(n: i32): i32 { var r: Result[i32[], i32] = Ok([10, 20, 30]); var c = 0; if (n > 0) { match (r) { Ok(v) => { c = v[0] + v[2]; }, Err(e) => { c = e; } } } return c + 2 + __rc_underflow_count(); } function main(): i32 { return f(5); }`, 42},
	}
