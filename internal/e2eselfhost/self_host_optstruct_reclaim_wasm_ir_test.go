...
func TestSelfHostOptStructReclaimWasmIR(t *testing.T) {
	if _, err := exec.LookPath("wasmtime"); err != nil {
		t.Skip("wasmtime not on PATH; skipping optstruct-reclaim wasm IR e2e")
	}
	...
	for _, tc := range optStructReclaimCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := rcmd.ProcessState.ExitCode(); got != tc.want {
				t.Errorf("%s = %d, want %d (98 = option leaked; 99 = over-release/underflow; 97 = value corrupted)", tc.name, got, tc.want)
			}
		})
	}
}
