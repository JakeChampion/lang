...
func TestSelfHostTupleStructReclaimWasmIR(t *testing.T) {
	if _, err := exec.LookPath("wasmtime"); err != nil {
		t.Skip("wasmtime not on PATH; skipping tuplestruct-reclaim wasm IR e2e")
	}
	...
	for _, tc := range tupleStructReclaimCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := rcmd.ProcessState.ExitCode(); got != tc.want {
				t.Errorf("%s = %d, want %d (98 = tuple/struct leaked; 99 = over-release/underflow; 97 = value corrupted)", tc.name, got, tc.want)
			}
		})
	}
}
