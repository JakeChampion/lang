...
// kind with a pop count ir.op_pops does not model cannot be bridged, and
// dyn_dispatch reads its arguments from frame slots the register path does
// not keep. A new line in the report is a kind the register path declines on
// every program that uses it, which is what this gate exists to refuse.
func TestSelfHostSSALiftAdmitsEveryOp(t *testing.T) {
	...
	const want = "load pops=-1 pushes=1\n" +
		...
		"dyn_dispatch pops=0 pushes=1\n" +
		"call_closure_direct pops=-1 pushes=1\n" +
		"registered=308 declined=4\n"
