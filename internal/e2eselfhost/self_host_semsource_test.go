...
function main(): i32 {
    ...
    for fd in mod.funcs {
        ...
        if (fd.name == "main" || p.template) {
            ...
            for row in ssaunits.grow_rows(fd.name, p.func, plan) { grows = grows.append(row); }
        }
    ...
    for p in built.instances {
        ...
        for row in ssaunits.grow_rows(p.func.graph.name, p.func, plan) { grows = grows.append(row); }
        plans = plans.append(plan);
    }
    at = 0;
    for fd in mod.funcs {
