...
func unitSource(indices []int) (string, string) {
	...
	for _, i := range indices {
		...
		source.WriteString(`
var f = ssasem.Func { envs: [], anchors: [], graph: graph, values: types, params: params, result: result, records: records, enums: enums, calls: calls };
...
for opaque in opaque_types {
    ...
    bad = ssaunits.plan(ssasem.Func { envs: [], anchors: [], graph: g, values: [opaque], params: [opaque], result: opaque, records: [], enums: [], calls: [] }, [3]);
