...
func TestSelfHostIRKindRegistry(t *testing.T) {
	...
	// round-trip), the extension-tag sweep (the 51 registered ids beyond
	// kind_count(), struct_copy=198 … cpu_count=252 — #5452's skew
	...
	// via kind_name, the 51 extension tags by name — so the round trip this
	...
	// proves every one of the 247 tags round-tripped AND that no near miss
