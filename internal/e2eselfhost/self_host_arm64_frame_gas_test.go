...
// Structural everywhere; executed on Apple Silicon. No external tool.
func TestSelfHostArm64DarwinMachOFnGasRuns(t *testing.T) {
	if _, err := exec.LookPath("wasmtime"); err != nil {
		t.Skip("wasmtime not on PATH; skipping self-host arm64-darwin fn gas run")
	}
	gcc, runner := x86_64Tooling(t)
	...
	source := arm64NativeSrc(t) + "\n" + arm64MachOFnGasDriverMain

	wat := runCapture(t, gcc, runner, driverBin, []byte(source))
	if len(wat) == 0 {
		t.Fatal("wasm emitter produced 0 bytes for the arm64-darwin fn gas driver")
	}
	watPath := filepath.Join(dir, "arm64_macho_fn_gas_driver.wat")
	if err := os.WriteFile(watPath, wat, 0o644); err != nil {
		t.Fatalf("write wat: %v", err)
	}
	...
	binPath := filepath.Join(dir, "fn42")
	...
	if got := ps.ExitCode(); got != 42 {
		t.Errorf("self-host arm64-darwin fn gas exit = %d, want 42", got)
	}
}
