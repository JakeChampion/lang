...
func TestSelfHostRcStructBoxWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"struct-holds-array", "struct B { xs: i32[], n: i32 } function main(): i32 { var ys: i32[] = [1, 2, 3]; var b = B { xs: ys, n: 9 }; return b.xs[2] + b.n + __rc_underflow_count(); }", 12},
		// A struct holding a string field: value intact, detector clean.
		{"struct-holds-string", "struct N { name: string, n: i32 } function main(): i32 { var s: string = \"ab\" + \"cd\"; var v = N { name: s, n: 5 }; return v.name.len() + v.n + __rc_underflow_count(); }", 9},
		...
		{"struct-return-intact", "struct P { x: i32, y: i32 } function mk(): P { return P { x: 8, y: 34 }; } function main(): i32 { var p = mk(); return p.x + p.y + __rc_underflow_count(); }", 42},
		// A struct built each loop iteration: detector stays clean (layout +
		// construction-incs only; free off, so this leaks soundly).
		{"struct-loop-clean", "struct P { x: i32, y: i32 } function main(): i32 { var s = 0; var k = 0; while (k < 1000) { var p = P { x: k, y: 2 }; s = s + p.y; k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 5},
		// COUNTING milestone (free off): an owned struct local is released
		// (rc dec) at exit, value-correct + detector clean.
		{"struct-swept-clean", "struct P { x: i32, y: i32 } function main(): i32 { var p = P { x: 5, y: 7 }; return p.x + p.y + __rc_underflow_count(); }", 12},
		// Aliasing a struct: the alias is inc'd, both swept, balanced.
		{"struct-alias-clean", "struct P { x: i32, y: i32 } function main(): i32 { var p = P { x: 3, y: 4 }; var u = p; return u.x + p.y + __rc_underflow_count(); }", 7},
		...
		{"struct-move-return-clean", "struct P { x: i32, y: i32 } function mk(): P { return P { x: 8, y: 34 }; } function main(): i32 { var p = mk(); var q = mk(); return p.x + q.y + __rc_underflow_count(); }", 42},
		...
		{"struct-borrowed-field-return", "struct Inner { v: i32 } struct Outer { inner: Inner, n: i32 } function get_inner(o: Outer): Inner { return o.inner; } function main(): i32 { var i = Inner { v: 9 }; var o = Outer { inner: i, n: 5 }; var r = get_inner(o); return r.v + o.inner.v + __rc_underflow_count(); }", 18},
		...
		{"struct-base-copy-churn-clean", "struct Acc { xs: i32[], n: i32 } function step(a: Acc): Acc { return Acc { ...a, n: a.n + 1 }; } function main(): i32 { var a = Acc { xs: [1, 2, 3], n: 0 }; var k = 0; while (k < 1000) { a = step(a); k = k + 1; } return (a.n % 7) + a.xs[2] + __rc_underflow_count(); }", 9},
		...
		{"struct-field-array-released", "struct B { xs: i32[], n: i32 } function main(): i32 { var ys: i32[] = [1, 2, 3]; var b = B { xs: ys, n: 9 }; return b.xs[2] + b.n + __rc_underflow_count(); }", 12},
		// Same for a string field.
		{"struct-field-string-released", "struct N { name: string, n: i32 } function main(): i32 { var s: string = \"ab\" + \"cd\"; var v = N { name: s, n: 5 }; return v.name.len() + v.n + __rc_underflow_count(); }", 9},
		...
		{"struct-nested-released", "struct Inner { v: i32 } struct Outer { inner: Inner, n: i32 } function main(): i32 { var i = Inner { v: 7 }; var o = Outer { inner: i, n: 5 }; return o.inner.v + o.n + __rc_underflow_count(); }", 12},
		...
		{"struct-builder-escape-clean", "struct Inner { v: i32 } struct Outer { inner: Inner, n: i32 } function mk(): Outer { var i = Inner { v: 9 }; return Outer { inner: i, n: 5 }; } function main(): i32 { var o = mk(); var p = mk(); return o.inner.v + p.n + __rc_underflow_count(); }", 14},
		...
		{"struct-array-churn-clean", "struct B { xs: i32[], n: i32 } function mk(): i32 { var a: i32[] = [1, 2, 3, 4, 5, 6, 7, 8]; var b = B { xs: a, n: 5 }; return b.xs[7] + b.n; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 6},
		...
		// detector clean.
		{"struct-field-strarray-released", "struct Bag { items: string[], n: i32 } function main(): i32 { var xs: string[] = [\"a\" + \"b\", \"c\" + \"d\"]; var bag = Bag { items: xs, n: 5 }; return bag.items[0].len() + bag.items[1].len() + bag.n + __rc_underflow_count(); }", 9},
		...
		{"struct-field-i32array-flat-safe", "struct C { ns: i32[], k: i32 } function main(): i32 { var ns: i32[] = [262184, 262192, 262200]; var c = C { ns: ns, k: 3 }; return c.ns.len() + c.k + __rc_underflow_count(); }", 6},
		...
		{"struct-field-strarray-churn-clean", "struct Bag { items: string[], n: i32 } function mk(): i32 { var bag = Bag { items: [\"x\" + \"y\", \"z\" + \"w\"], n: 4 }; return bag.items[0].len() + bag.n; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 6},
		...
		{"struct-rebind-loop-reclaim", "struct P { x: i32, y: i32 } function main(): i32 { var n = 0; var k = 0; while (k < 100000) { var p = P { x: k, y: 2 }; n = n + p.y; k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 3},
		...
		{"struct-reassign-base-copy-reclaim", "struct St { names: string[], n: i32 } function step(s: St): St { return St { ...s, n: s.n + 1 }; } function main(): i32 { var s = St { names: [\"a\" + \"b\"], n: 0 }; var k = 0; while (k < 100000) { s = step(s); k = k + 1; } return (s.n % 7) + s.names[0].len() + __rc_underflow_count(); }", 7},
	}
