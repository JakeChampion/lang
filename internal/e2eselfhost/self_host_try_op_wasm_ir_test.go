...
// try-operator (`inner?`) lowers in the target-independent irlower, so the wasm
...
func TestSelfHostTryOpWasmIR(t *testing.T) {
	if _, err := exec.LookPath("wasmtime"); err != nil {
		t.Skip("wasmtime not on PATH; skipping self-host try-op wasm IR e2e")
	}
	...
	for _, tc := range tryOpIRCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := rcmd.ProcessState.ExitCode(); got != tc.expected {
				t.Errorf("try-op wasm IR %q = %d, want %d", tc.name, got, tc.expected)
			}
		})
	}
}
