...
func TestSelfHostNativeTCPLifecycleCensus(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("requires Linux native targets")
	}
	checkSelfHostNativeTCPCensus(t, []string{"x86-64-linux", "arm64-linux"})
}

func TestSelfHostArm64DarwinTCPLifecycleCensus(t *testing.T) {
	if runtime.GOOS != "darwin" || runtime.GOARCH != "arm64" {
		t.Skip("requires native Apple Silicon")
	}
	checkSelfHostNativeTCPCensus(t, []string{"arm64-darwin"})
}

func checkSelfHostNativeTCPCensus(t *testing.T, targets []string) {
	t.Helper()
	dir := t.TempDir()
