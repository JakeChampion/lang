...
function main(): i32 {
    ...
    return t % 97;
		...
		{
			// The SOLE top-level match binding its payload out. That shape takes
			// the match-consumed branch, which match_arm_binds_rc_payload refuses,
			// and this slice does not touch it — 300/100 against native's 300/300,
			// pinned as the gap it is rather than as a decision.
			name: "sole_match_binds_payload_out_still_refused",
			src: decls + `function round(i: i32): i32 {
    var v: E = mkv(i);
    var keep: i32[] = [0];
