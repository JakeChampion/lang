...
func TestSelfHostStrSliceRcBoxIRX86_64(t *testing.T) {
	...
	run(t, `function main(): i32 { var s: string = "hello"; var t: string = slice_unchecked(s, 1, 4) + ""; return t.len(); }`,
	...
	// (rc=-1), so no double-free can tick __rc_underflow_count(); the shared source
	// buffer is never reclaimed out from under a live view. r = "mid" len 3, so bad stays
	// 0, underflow 0 -> exit 0.
	run(t, `function churn(n: i32): i32 { var s: string = "  mid  "; var bad: i32 = 0; var i: i32 = 0; while (i < n) { var r: string = s.trim(); if (r.len() != 3) { bad = 1; } var sl: string = slice_unchecked(s, 2, 5); if (sl.len() != 3) { bad = 1; } i = i + 1; } return bad; } function main(): i32 { var v: i32 = churn(2000000); if (__rc_underflow_count() != 0) { return 99; } return v; }`,
