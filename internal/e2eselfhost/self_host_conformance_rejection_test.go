...
//
// What the blind spot was hiding: on 11 of the 69 the self-host checker reports
// NO diagnostic at all. `err_map_key_no_hash` is one, and it shows what a
// missing rejection costs once it reaches a backend — the wasm module it
// produces does not validate, calling a `$Has.hash` nothing defines.
//
...
//
//   - a case NOT in the gap file must be rejected — the 53 that work today
//     (69 cases, less the 11 gaps and the 5 `parse:` rows) can never silently
//     regress to accepted;
