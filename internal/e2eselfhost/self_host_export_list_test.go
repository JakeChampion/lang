...
// that @imports `iota() -> list<s32>` reads the array back — linked with
// `wasm-tools compose` and run under wasmtime. docs/WIT-BRING-YOUR-OWN.md.
func TestSelfHostExportListResultRunsViaConsumer(t *testing.T) {
...
function main(): i32 { return 0; }`
	...
	if !bytes.Contains(watBytes, []byte("local:test/nums@0.1.0#iota")) {
		t.Fatalf("self-host core missing the surfaced list @export:\n%s", watBytes)
	}
	...
	if err := os.WriteFile(filepath.Join(expWit, "deps", "test", "nums.wit"),
		[]byte("package local:test@0.1.0;\ninterface nums { iota: func() -> list<s32>; }\n"), 0o644); err != nil {
	...
	if err != nil {
		t.Fatalf("ComposeExportsFromWorld (self-host list export): %v", err)
	}
	...
	// Go-built consumer that imports iota() -> list<s32> and checks it.
	...
	const want = "iota-ok"
	userSrc := `@import("local:test/nums@0.1.0", "iota")
function iota(): i32[];
