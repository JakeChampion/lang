...
func TestSelfHostRcAliasIncX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"alias-no-underflow", "function main(): i32 { var xs: i32[] = [5, 6, 7]; var ys = xs; var zs = ys; return __rc_underflow_count(); }", 0},
...
func TestSelfHostRcReassignX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"reassign-no-underflow", "function main(): i32 { var xs: i32[] = [1, 2]; var ys: i32[] = [3, 4]; ys = xs; var zs: i32[] = [5, 6]; zs = ys; return __rc_underflow_count(); }", 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcExitSweepX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"borrowed-param", "function sum2(a: i32[]): i32 { return a[0] + a[1]; } function main(): i32 { var xs: i32[] = [7, 8]; var r = sum2(xs); return r + xs[0] + __rc_underflow_count(); }", 22},
		...
		{"exit-sweep-no-underflow", "function f(): i32 { var xs: i32[] = [1, 2]; var ys = xs; return ys[0]; } function main(): i32 { var a = f(); var b = f(); var c = f(); return __rc_underflow_count(); }", 0},
		...
		{"branch-local-unbound", "function main(): i32 { var xs: i32[] = [5, 6]; if (xs[0] > 100) { var ys: i32[] = [1, 2]; return ys[0]; } return xs[1] + __rc_underflow_count(); }", 6},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcMoveOnReturnX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"move-bare-local", "function make(): i32[] { var xs: i32[] = [10, 20, 30]; return xs; } function main(): i32 { var ys = make(); return ys[0] + ys[2] + __rc_underflow_count(); }", 40},
		// Moved through a chain of callers (each return moves), detector clean.
		{"move-chained", "function mk(): i32[] { var xs: i32[] = [1, 2, 3]; return xs; } function relay(): i32[] { var a = mk(); return a; } function main(): i32 { var b = relay(); return b[0] + b[2] + __rc_underflow_count(); }", 4},
		...
		{"move-with-sibling-sweep", "function make(): i32[] { var keep: i32[] = [9, 9]; var xs: i32[] = [5, 6, 7]; return xs; } function main(): i32 { var ys = make(); return ys[0] + ys[2] + __rc_underflow_count(); }", 12},
		// Returning a BORROWED param array is NOT a move (idx < n_params):
		// the caller still owns it, so it stays usable after the call.
		{"return-param-not-moved", "function pick(a: i32[]): i32[] { return a; } function main(): i32 { var xs: i32[] = [3, 4]; var ys = pick(xs); return ys[0] + xs[1] + __rc_underflow_count(); }", 7},
		...
		{"move-churn", "function build(n: i32): i32[] { var xs: i32[] = []; var i = 0; while (i < n) { xs = xs.append(i); i = i + 1; } return xs; } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { var r = build(64); s = r[63]; k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcArm64(t *testing.T) {
	...
	cases := []struct {
		...
		{"self-append-no-underflow", "function main(): i32 { var xs: i32[] = []; var i = 0; while (i < 20) { xs = xs.append(i); i = i + 1; } return __rc_underflow_count(); }", 0},
		...
		{"struct-holds-array", "struct H { items: i32[] } function mk(): H { var xs: i32[] = [7, 8]; return H { items: xs }; } function main(): i32 { var h = mk(); return h.items[0] + h.items[1] + __rc_underflow_count(); }", 15},
		{"array-of-arrays", "function main(): i32 { var a: i32[] = [1, 2]; var b: i32[] = [3, 4]; var both: i32[][] = [a, b]; return both[0][1] + both[1][0] + __rc_underflow_count(); }", 5},
		{"struct-update-copy", "struct H { items: i32[], n: i32 } function main(): i32 { var xs: i32[] = [1, 2]; var h: H = H { items: xs, n: 0 }; var h2: H = H { ...h, n: 5 }; return h2.items[1] + h2.n + __rc_underflow_count(); }", 7},
		// Phase 3 (arm64 free): reclamation churn (alloc >> heap completes) + enum payload retain.
		{"reclaim-churn", "function work(n: i32): i32 { var xs: i32[] = []; var i = 0; while (i < n) { xs = xs.append(i); i = i + 1; } return xs[n - 1]; } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = work(200); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 3},
		{"enum-holds-array", "enum Box { Arr(i32[]), Empty } function mk(): Box { var xs: i32[] = [3, 4, 5]; return Arr(xs); } function main(): i32 { var b = mk(); match (b) { Arr(a) => { return a[1] + a[2] + __rc_underflow_count(); }, Empty => { return 0; } } }", 9},
...
func TestSelfHostRcSelfMutateX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"reassign-different-clean", "function main(): i32 { var xs: i32[] = [1, 2]; var ys: i32[] = [3, 4]; ys = xs; return ys[0] + xs[1] + __rc_underflow_count(); }", 3},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcConstructX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"struct-holds-array", "struct H { items: i32[] } function main(): i32 { var xs: i32[] = [1, 2, 3]; var h: H = H { items: xs }; return h.items[1] + xs[0] + __rc_underflow_count(); }", 3},
		...
		{"struct-outlives-source", "struct H { items: i32[] } function mk(): H { var xs: i32[] = [7, 8]; return H { items: xs }; } function main(): i32 { var h = mk(); return h.items[0] + h.items[1] + __rc_underflow_count(); }", 15},
		// A struct field from a fresh literal is owned — not re-incremented.
		{"struct-fresh-literal", "struct H { items: i32[] } function main(): i32 { var h: H = H { items: [4, 5, 6] }; return h.items[2] + __rc_underflow_count(); }", 6},
		// Struct-update copies the base's array field (retained for soundness).
		{"struct-update-copy", "struct H { items: i32[], n: i32 } function main(): i32 { var xs: i32[] = [1, 2]; var h: H = H { items: xs, n: 0 }; var h2: H = H { ...h, n: 5 }; return h2.items[1] + h2.n + __rc_underflow_count(); }", 7},
		{"struct-update-override", "struct H { items: i32[], n: i32 } function main(): i32 { var xs: i32[] = [9, 8]; var h: H = H { items: [0], n: 1 }; var h2: H = H { ...h, items: xs }; return h2.items[0] + __rc_underflow_count(); }", 9},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcConstructContainersX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"tuple-of-array", "function main(): i32 { var xs: i32[] = [7, 8]; var t = (xs, 9); return t.0[1] + t.1 + __rc_underflow_count(); }", 17},
		// Returning a container that captured a local array (would UAF
		// once free is on without the construction inc) stays correct.
		{"return-arr-of-arrs", "function mk(): i32[][] { var a: i32[] = [5, 6]; return [a, a]; } function main(): i32 { var both = mk(); return both[0][0] + both[1][1] + __rc_underflow_count(); }", 11},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcClosureX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"closure-captures-array", "function main(): i32 { var xs: i32[] = [3, 4, 5]; var f = (): i32 => { return xs[1] + xs[2]; }; return f() + __rc_underflow_count(); }", 9},
		// Closure escaping its defining function, capturing an array.
		{"closure-escapes-with-array", "function mk(xs: i32[]): () => i32 { return (): i32 => { return xs[0] + xs[1]; }; } function main(): i32 { var a: i32[] = [3, 4]; var f = mk(a); return f() + __rc_underflow_count(); }", 7},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcFreeReclaimX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"reclaim-churn", "function work(n: i32): i32 { var xs: i32[] = []; var i = 0; while (i < n) { xs = xs.append(i); i = i + 1; } return xs[n - 1]; } function main(): i32 { var k = 0; var s = 0; while (k < 500000) { s = work(200); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 3},
		// Borrowed-param builder: callee must not free the caller's buffer.
		{"borrowed-param-builder", "function add(toks: i32[], t: i32): i32[] { return toks.append(t); } function main(): i32 { var ts: i32[] = []; var i = 0; while (i < 200) { ts = add(ts, i); i = i + 1; } return ts[199] + __rc_underflow_count(); }", 199},
		...
		{"loop-local-rebind", "function build(n: i32): i32[] { var xs: i32[] = []; var i = 0; while (i < n) { xs = xs.append(i); i = i + 1; } return xs; } function main(): i32 { var s = 0; var k = 0; while (k < 100000) { var r: i32[] = build(32); s = s + r[31]; k = k + 1; } return (s % 5) + __rc_underflow_count(); }", 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			asm := runCapture(t, gcc, runner, driverBin, []byte(tc.src))
...
func TestSelfHostRcStructArrayFieldDropX86_64(t *testing.T) {
	...
	cases := []struct {
		...
		{"struct-arr-field-fresh-no-underflow", "struct E { v: i32 } struct H { es: E[] } function step(n: i32): i32 { var h = H { es: [E { v: n }, E { v: n + 1 }] }; return h.es[0].v; } function main(): i32 { var s = 0; var i = 0; while (i < 2000) { s = s + step(i); i = i + 1; } return s - s + __rc_underflow_count(); }", 0},
		...
		{"struct-arr-field-alias-no-underflow", "struct E { v: i32 } struct H { es: E[] } function wrapH(src: E[]): i32 { var h = H { es: src }; return h.es[0].v; } function main(): i32 { var shared: E[] = [E { v: 3 }, E { v: 4 }]; var s = 0; var i = 0; while (i < 2000) { s = s + wrapH(shared); i = i + 1; } return s - s + __rc_underflow_count(); }", 0},
		...
		{"struct-arr-field-callvalue-no-underflow", "struct E { v: i32 } struct H { es: E[] } function mk(n: i32): E[] { return [E { v: n }, E { v: n * 2 }]; } function step(n: i32): i32 { var h = H { es: mk(n) }; return h.es[1].v; } function main(): i32 { var s = 0; var i = 0; while (i < 2000) { s = s + step(i); i = i + 1; } return s - s + __rc_underflow_count(); }", 0},
		// Array-of-ENUM field: the buffer is reclaimed the same shallow way.
		{"enum-arr-field-no-underflow", "enum K { A(i32), B } struct G { ks: K[] } function step(n: i32): i32 { var g = G { ks: [A(n), B] }; return match (g.ks[0]) { A(x) => x, B => 0 }; } function main(): i32 { var s = 0; var i = 0; while (i < 2000) { s = s + step(i); i = i + 1; } return s - s + __rc_underflow_count(); }", 0},
