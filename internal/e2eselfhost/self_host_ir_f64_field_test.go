...
// results: (a) read, (b) mixed i32/f64 fields, (c) functional update — all eligible →
// 4+2+1 = 7.
func TestSelfHostIRF64FieldEligible(t *testing.T) {
	progs := []string{
		...
		"struct P { x: f64, n: i32 } function main(): i32 { var p = P { x: 1.0, n: 4 }; var q = P { ...p, x: 5.5 }; if (q.x > 5.0) { return q.n; } return 0; }",
	}
