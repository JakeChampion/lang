...
func TestSelfHostRcDeepNestWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"deep-nested-2", "struct Inner { xs: i32[], n: i32 } struct Outer { inner: Inner, m: i32 } function main(): i32 { var i = Inner { xs: [1, 2, 3], n: 5 }; var o = Outer { inner: i, m: 9 }; return o.inner.xs[2] + o.inner.n + o.m + __rc_underflow_count(); }", 17},
		// 3-level transitive: C -> B -> A -> A.v array.
		{"deep-nested-3", "struct A { v: i32[] } struct B { a: A, x: i32 } struct C { b: B, y: i32 } function main(): i32 { var a = A { v: [7, 8] }; var b = B { a: a, x: 1 }; var c = C { b: b, y: 2 }; return c.b.a.v[1] + c.b.x + c.y + __rc_underflow_count(); }", 11},
		// A nested STRING field deep in the tree is released too.
		{"deep-nested-string", "struct In { s: string } struct Out { in: In, n: i32 } function main(): i32 { var i = In { s: \"ab\" + \"cd\" }; var o = Out { in: i, n: 38 }; return o.in.s.len() + o.n + __rc_underflow_count(); }", 42},
		...
		{"deep-nested-churn", "struct Inner { xs: i32[], n: i32 } struct Outer { inner: Inner, m: i32 } function mk(): i32 { var i = Inner { xs: [1, 2, 3, 4, 5, 6, 7, 8], n: 5 }; var o = Outer { inner: i, m: 9 }; return o.inner.xs[7] + o.m; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 3},
		...
		{"deep-builder-escape", "struct Inner { xs: i32[], n: i32 } struct Outer { inner: Inner, m: i32 } function mk(): Outer { var i = Inner { xs: [3, 4, 5], n: 6 }; return Outer { inner: i, m: 7 }; } function main(): i32 { var o = mk(); var p = mk(); return o.inner.xs[1] + o.inner.n + o.m + __rc_underflow_count(); }", 17},
		...
		{"arr-of-struct-released", "struct Inner { xs: i32[], n: i32 } function main(): i32 { var ps: Inner[] = [Inner { xs: [1, 2], n: 3 }, Inner { xs: [4, 5], n: 6 }]; return ps[0].xs[1] + ps[1].xs[0] + ps[1].n + __rc_underflow_count(); }", 12},
		...
		{"arr-of-struct-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var ps: Inner[] = [Inner { xs: [1, 2, 3, 4], n: 5 }, Inner { xs: [6, 7, 8, 9], n: 1 }]; return ps[0].xs[3] + ps[1].n; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 5},
		...
		{"enum-string-payload-released", "enum Shape { Circle(string), Square(i32) } function main(): i32 { var s: Shape = Circle(\"ab\" + \"cd\"); match (s) { Circle(name) => { return name.len() + 38 + __rc_underflow_count(); }, Square(w) => { return w; } } }", 42},
		// An enum array-payload variant releases its array.
		{"enum-array-payload-released", "enum Box { Has(i32[]), Empty } function main(): i32 { var b: Box = Has([10, 20, 30]); match (b) { Has(xs) => { return xs[1] + 22 + __rc_underflow_count(); }, Empty => { return 0; } } }", 42},
		...
		{"enum-payload-churn", "enum Shape { Circle(string), Square(i32) } function mk(): i32 { var s: Shape = Circle(\"x\" + \"y\"); match (s) { Circle(name) => { return name.len(); }, Square(w) => { return w; } } } function main(): i32 { var k = 0; var n = 0; while (k < 50000) { n = mk(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 2},
		...
		{"arr-of-enum-released", "enum Shape { Circle(string), Square(i32) } function main(): i32 { var shapes: Shape[] = [Circle(\"ab\" + \"cd\"), Square(7)]; match (shapes[0]) { Circle(name) => { return name.len() + 38 + __rc_underflow_count(); }, Square(w) => { return w; } } }", 42},
		...
		{"arr-of-enum-churn", "enum Shape { Circle(string), Square(i32) } function mk(): i32 { var shapes: Shape[] = [Circle(\"x\" + \"y\"), Circle(\"z\" + \"w\"), Square(3)]; match (shapes[1]) { Circle(name) => { return name.len(); }, Square(w) => { return w; } } } function main(): i32 { var k = 0; var n = 0; while (k < 50000) { n = mk(); k = k + 1; } return (n % 7) + __rc_underflow_count(); }", 2},
		...
		{"tuple-enum-payload-released", "enum Shape { Circle(string), Square(i32) } function main(): i32 { var s: Shape = Circle(\"ab\" + \"cd\"); var t = (s, 38); match (t.0) { Circle(name) => { return name.len() + t.1 + __rc_underflow_count(); }, Square(w) => { return w; } } }", 42},
		...
		{"tuple-struct-element-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var i = Inner { xs: [1, 2, 3, 4], n: 5 }; var t = (i, 9); return t.1; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 2},
		...
		{"arr-tuple-string-released", "function main(): i32 { var ps: (i32, string)[] = [(1, \"ab\" + \"cd\"), (2, \"ef\" + \"gh\")]; return ps[0].0 + ps[1].0 + ps[0].1.len() + 35 + __rc_underflow_count(); }", 42},
		...
		{"arr-tuple-string-churn", "function mk(): i32 { var ps: (i32, string)[] = [(1, \"ab\" + \"cd\"), (2, \"ef\" + \"gh\")]; return ps[0].0 + ps[1].0 + ps[0].1.len() + ps[1].1.len(); } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 4},
		...
		{"arr-tuple-arrelem-churn", "function mk(): i32 { var ps: (i32, i32[])[] = [(1, [10, 20]), (2, [30, 40])]; return ps[0].0 + ps[1].1[0]; } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 3},
		...
		{"arr-tuple-struct-elem-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var ps: (Inner, i32)[] = [(Inner { xs: [1, 2, 3], n: 4 }, 7), (Inner { xs: [5, 6], n: 8 }, 9)]; return ps[0].1 + ps[1].1; } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __rc_underflow_count(); }", 2},
	}
