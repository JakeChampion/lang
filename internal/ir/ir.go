...
func lowerFunc(fn *ast.FuncDecl, info *checker.Info, ptrW int, dynRcSupported bool, pairForm map[string]bool, closureCaps map[string][]ast.Param, genEnumDrops map[string]*ast.EnumDecl, genTupleDrops map[string]ast.TupleType, returnsNoParamEscape map[string]bool, trmcFuncs, trmcConsumeSafe map[string]bool, paramEscapes map[string][]bool, readOnlyComparators map[string]bool) (*Func, error) {
	...
	// Phase 1d covered arrays; the zero set has since widened to the full
	// zero-init class (rcZeroInitClass — pointer-shaped slots plus strings),
	// the same capability axis the exit dec sweep reads, so the safety zero
	// and the dec agree on which slots they touch and the sweep's `ptr == 0`
	// null guard fires on never-initialised locals (conditional / match-arm
	// declarations).
	for _, v := range info.Locals[fn] {
		if !rcZeroInitClass(v.Type) {
			continue
		}
		if zeroSeen[v.Name] {
			continue
