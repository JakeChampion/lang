...
func TestRunnerUrlExamplePasses(t *testing.T) {
	...
	for _, w := range []string{"# Suite: std/url", "# pass 10", "# fail 0", "1..10"} {
		if !strings.Contains(out, w) {
			t.Errorf("stdout missing %q\nfull output:\n%s", w, out)
		}
	}
...
func TestRunnerSortByAndCiExamplePasses(t *testing.T) {
	...
	for _, w := range []string{"# Suite: std/sort comparator + ci", "# pass 9", "# fail 0", "1..9"} {
		if !strings.Contains(out, w) {
			t.Errorf("stdout missing %q\nfull output:\n%s", w, out)
		}
	}
...
func TestRunnerStringClassifyTransformExamplePasses(t *testing.T) {
	...
	for _, w := range []string{"# Suite: std/string classify + transform", "# pass 17", "# fail 0", "1..17"} {
		if !strings.Contains(out, w) {
			t.Errorf("stdout missing %q\nfull output:\n%s", w, out)
		}
	}
...
func TestRunnerStringSliceExtractExamplePasses(t *testing.T) {
	...
	for _, w := range []string{"# Suite: std/string slice + extract", "# pass 19", "# fail 0", "1..19"} {
		if !strings.Contains(out, w) {
			t.Errorf("stdout missing %q\nfull output:\n%s", w, out)
		}
	}
...
func TestRunnerJsonRoundtripExamplePasses(t *testing.T) {
	...
	for _, w := range []string{"# Suite: std/json roundtrip", "# pass 11", "# fail 0", "1..11"} {
		if !strings.Contains(out, w) {
			t.Errorf("stdout missing %q\nfull output:\n%s", w, out)
		}
	}
...
// per-branch continuations), and a guard-clause `await` with fall-through
// (slice 3b). Passing suite → exit 0.
func TestRunnerAsyncTaskFnExamplePasses(t *testing.T) {
	...
	if !strings.Contains(out, "# pass 8") || !strings.Contains(out, "# fail 0") {
		t.Errorf("expected 8 passes, 0 fails\noutput:\n%s", out)
	}
}
