...
func TestSelfHostAsmIRPath(t *testing.T) {
	...
	cases := []struct {
		...
		// irlower.lift_lambdas, so it lowers through the IR path's
		// StmtWhile/StmtIf/StmtBreak — not the AST fallback). Covers the
		// step, continue (re-runs the increment), break, empty/reversed (zero
		// iterations), nested loops, and a call-once HIGH bound.
		...
		{"range-in-while", `function main(): i32 { var t = 0; var k = 0; while (k < 3) { for i in 0..3 { t = t + 1; } k = k + 1; } return t; }`},
