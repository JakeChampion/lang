...
func TestSelfHostRcArrElemWasm(t *testing.T) {
	...
	cases := []struct {
		...
		{"str-array-elem-released", "function main(): i32 { var a: string = \"a\" + \"x\"; var b: string = \"b\" + \"y\"; var arr: string[] = [a, b]; return arr[0].len() + arr[1].len() + __fern_rc_underflow_count(); }", 4},
		...
		{"str-array-elem-aliased-clean", "function main(): i32 { var s: string = \"ab\" + \"cd\"; var arr: string[] = [s]; return arr[0].len() + s.len() + __fern_rc_underflow_count(); }", 8},
		// Appended (not literal) heap strings are released too (the .append
		// path retains, the exit sweep recursively releases).
		{"str-array-append-released", "function main(): i32 { var arr: string[] = []; var i = 0; while (i < 3) { arr = arr.append(\"x\" + \"y\"); i = i + 1; } return arr.len() + arr[2].len() + __fern_rc_underflow_count(); }", 5},
		...
		{"str-array-churn-clean", "function mk(): i32 { var arr: string[] = [\"a\" + \"b\", \"c\" + \"d\", \"e\" + \"f\"]; return arr[0].len() + arr[1].len() + arr[2].len(); } function main(): i32 { var k = 0; var s = 0; while (k < 50000) { s = mk(); k = k + 1; } return (s % 7) + __fern_rc_underflow_count(); }", 6},
		// A struct-array freed releases each struct box (one level).
		{"struct-array-elem-released", "struct P { x: i32, y: i32 } function main(): i32 { var ps: P[] = [P { x: 1, y: 2 }, P { x: 3, y: 4 }]; return ps[0].x + ps[1].y + __fern_rc_underflow_count(); }", 5},
		// A struct-array where a struct element is also a surviving local: no
		// over-release.
		{"struct-array-elem-aliased-clean", "struct P { x: i32, y: i32 } function main(): i32 { var p = P { x: 5, y: 6 }; var ps: P[] = [p]; return ps[0].x + p.y + __fern_rc_underflow_count(); }", 11},
		...
		{"str-array-aliased-buffer-clean", "function main(): i32 { var a: string = \"p\" + \"q\"; var xs: string[] = [a]; var ys: string[] = xs; return ys[0].len() + xs[0].len() + __fern_rc_underflow_count(); }", 4},
	}
