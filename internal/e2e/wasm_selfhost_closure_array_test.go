...
func TestWasmSelfHostClosureArray(t *testing.T) {
	...
	cases := []struct {
		...
		{"index-call", "function mk(s: i32): () => i32 { return (): i32 => { return s; }; } function main(): i32 { var fns: (() => i32)[] = [mk(42), mk(9)]; var c = fns[0]; return c(); }", 42},
		// Index the second element; the closure captures the call arg.
		{"index-arg", "function adder(n: i32): (i32) => i32 { return (x: i32): i32 => { return x + n; }; } function main(): i32 { var fs: ((i32) => i32)[] = [adder(10), adder(20)]; var g = fs[1]; return g(22); }", 42},
		// Iterate the array and call each closure, summing the results.
		{"for-each-call", "function mk(s: i32): () => i32 { return (): i32 => { return s; }; } function main(): i32 { var fns: (() => i32)[] = [mk(10), mk(20), mk(12)]; var s: i32 = 0; for f in fns { s = s + f(); } return s; }", 42},
		...
		{"lambda-elems", "function main(): i32 { var n: i32 = 5; var fns: (() => i32)[] = [(): i32 => { return n + 1; }, (): i32 => { return n * 2; }]; var a = fns[0]; var b = fns[1]; return a() + b(); }", 16},
		...
		// fs[0](7) = 7 + n = 7 + 35 = 42.
		{"inline-cap-direct-index", "function main(): i32 { var n: i32 = 35; var fs = [(x: i32): i32 => { return x + n; }]; return fs[0](7); }", 42},
		...
		{"inline-cap-multi-forin", "function main(): i32 { var a: i32 = 1; var b: i32 = 10; var fs = [(x: i32): i32 => { return x + a; }, (x: i32): i32 => { return x * b; }]; var s = 0; for f in fs { s = s + f(10); } return s; }", 111},
	}
