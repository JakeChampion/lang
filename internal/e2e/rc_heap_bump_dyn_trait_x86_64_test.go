...
//
// x86-64 only — arm64 still leaks `dyn` (slice 4c, harmless per §4.4), so
// no arm64 counterpart here; the broader native dyn dispatch/downcast
// tests stay green on every backend.
package e2e
