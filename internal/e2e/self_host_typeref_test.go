...
// still caught. It prints a deterministic report and exits with the round-trip
// failure count (0 on a clean sweep). This pins the parse/render contract end-
...
func TestSelfHostTypeRef(t *testing.T) {
	...
	dir := t.TempDir()
	for _, name := range []string{"util.fern", "lexer.fern", "parser.fern", "typeref_run.fern"} {
