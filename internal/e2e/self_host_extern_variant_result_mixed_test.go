...
// TestSelfHostExternVariantResultMixedCustomProvider is the self-host port of
// the mixed (payloaded + payloadless) variant result (docs/WIT-BRING-YOUR-OWN.md,
// Go-side TestExternVariantResultMixedCustomProvider). A `variant opt-num
// { some(s32), none }` returns indirectly (disc:u8 @0, payload @4); the self-host
// wrapper materializes a `[struct_id@0][payload@4]` box for every case — the
// payloadless `none` case gets that box with an unused payload (tag-correct,
// match-correct). Verifies the self-host result gate accepts payloadless cases.
//
// The provider exports `lookup: func(n: s32) -> opt-num`: n>=0 → some(n*2),
// else none. The self-host program matches Found(v)->v / Missing->-1.
func TestSelfHostExternVariantResultMixedCustomProvider(t *testing.T) {
	...
	const iface = "interface src { variant opt-num { some(s32), none } lookup: func(n: s32) -> opt-num; }"
  ...
  (func (export "cabi_realloc") (param $op i32) (param $os i32) (param $al i32) (param $ns i32) (result i32)
    ...
    (local.get $p))
  (func (export "local:test/src@0.1.0#lookup") (param $n i32) (result i32)
    (local $r i32)
    ...
    (if (i32.ge_s (local.get $n) (i32.const 0))
      (then
        ...
        (i32.store offset=4 (local.get $r) (i32.mul (local.get $n) (i32.const 2))))
      (else (i32.store8 (local.get $r) (i32.const 1))))
...
`
	if err := os.WriteFile(filepath.Join(dir, "vrm_run.fern"), []byte(driver), 0o644); err != nil {
		t.Fatalf("write driver: %v", err)
	}
	driverBin := buildSelfHostBin(t, gcc, dir, "vrm_run.fern", "vrm_run")

	const want = "opt-ok"
	prog := `enum OptNum { Found(i32), Missing }
@import("local:test/src@0.1.0", "lookup")
function lookup(n: i32): OptNum;
function code(n: i32): i32 {
    match (lookup(n)) {
        Found(v) => { return v; },
        Missing => { return 0 - 1; },
    }
...
function main(): i32 {
    if (code(21) == 42 && code(0) == 0 && code(0 - 5) == 0 - 1) {
        write("` + want + `");
    } else {
        write("opt-bad");
    }
