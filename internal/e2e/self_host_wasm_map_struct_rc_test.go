...
func TestSelfHostRcMapStructVal(t *testing.T) {
	...
	cases := []struct {
		...
		{"map-struct-val", "struct P { x: i32, y: i32 } function main(): i32 { var m = map_new_i32(8); m = m.insert(1, P { x: 40, y: 2 }); var p = m.get_or(1, P { x: 0, y: 0 }); return p.x + p.y + __fern_rc_underflow_count(); }", 42},
		...
		{"map-struct-deep-churn", "struct Inner { xs: i32[], n: i32 } function mk(): i32 { var m = map_new_i32(2); m = m.insert(1, Inner { xs: [1, 2, 3, 4], n: 5 }); m = m.insert(2, Inner { xs: [6, 7, 8, 9], n: 1 }); var p = m.get_or(1, Inner { xs: [0], n: 0 }); return p.n; } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = mk(); k = k + 1; } return (s % 7) + __fern_rc_underflow_count(); }", 5},
		// string-keyed + struct value: both key and value reclaim (via_ releases
		// keys too). value-correct + detector 0.
		{"map-strkey-structval", "struct P { x: i32, y: i32 } function main(): i32 { var m = map_new(8); var k: string = \"a\" + \"b\"; m = m.insert(k, P { x: 40, y: 2 }); var p = m.get_or(k, P { x: 0, y: 0 }); return p.x + p.y + __fern_rc_underflow_count(); }", 42},
		// enum-valued map: variant payload (heap string) deep-released via
		// via_Shape -> $__fern_release_Shape dispatch. 200k churn, no OOM.
		{"map-enum-deep-churn", "enum Shape { Circle(string), Square(i32) } function mk(): i32 { var m = map_new_i32(2); m = m.insert(1, Circle(\"a\" + \"b\")); m = m.insert(2, Circle(\"c\" + \"d\")); return 2; } function main(): i32 { var k = 0; var s = 0; while (k < 200000) { s = mk(); k = k + 1; } return (s % 7) + __fern_rc_underflow_count(); }", 2},
	}
