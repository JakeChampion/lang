...
var mapIterCases = []struct {
	...
	{"key-len-plus-value", "function main(): i32 { var m: Map[string,i32] = map_new(4); m.set(\"a\", 10); m.set(\"bb\", 20); var it: MapIter[string,i32] = m.iter(); var sum: i32 = 0; while (it.has_next()) { sum = sum + it.key().len() + it.value(); it.advance(); } return sum; }", 33},
	...
	{"string-values", "function main(): i32 { var m: Map[string,string] = map_new(4); m.set(\"k1\", \"abc\"); m.set(\"k2\", \"de\"); var it: MapIter[string,string] = m.iter(); var t: i32 = 0; while (it.has_next()) { t = t + it.value().len(); it.advance(); } return t; }", 5},
	{"count", "function main(): i32 { var m: Map[string,i32] = map_new(8); m.set(\"x\", 1); m.set(\"y\", 2); m.set(\"z\", 3); var it: MapIter[string,i32] = m.iter(); var c: i32 = 0; while (it.has_next()) { c = c + 1; it.advance(); } return c; }", 3},
}
