...
func TestSelfHostFixpointArm64(t *testing.T) {
	...
	dir := t.TempDir()
	for _, name := range []string{"util.fern", "astwalk.fern", "asmcore.fern", "lexer.fern", "parser.fern", "flatten.fern", "ir.fern", "irlower.fern", "asm_ir.fern", "asm_arm64_ir.fern", "asm_arm64.fern", "bundle_run_arm64.fern"} {
	...
	// The driver module is bundle_run_arm64 with its `import
	// "std/io"` retargeted to the bundled `io` module below — so the
	...
	asmArm64IrSrc, _ := os.ReadFile("../../examples/self_host/asm_arm64_ir.fern")
	...
	srcBundle.WriteString("///MODULE util\n")
	srcBundle.Write(utilSrc)
	srcBundle.WriteString("///MODULE astwalk\n")
	srcBundle.Write(astwalkSrc)
