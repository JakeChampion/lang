...
// 2^63 and (2^32-1)^2 are built by shift / multiplication rather than as bare
// literals, which would exceed the signed range the lexer stores numbers in.
const u64RootsProg = `
import "std/u64" as u64m;
function two63(): u64 { return (1 as u64) << (63 as u64); }
function main(): i32 {
    if ((100 as u64).sqrt_floor() != (10 as u64)) { return 1; }
    if ((99 as u64).sqrt_floor() != (9 as u64)) { return 2; }
    if (((4294967295 as u64) * (4294967295 as u64)).sqrt_floor() != (4294967295 as u64)) { return 3; }  // (2^32-1)^2
    if ((0 as u64).sqrt_floor() != (0 as u64)) { return 4; }
    if ((1 as u64).sqrt_floor() != (1 as u64)) { return 5; }
    if (!two63().is_power_of_2()) { return 6; }                             // 2^63
