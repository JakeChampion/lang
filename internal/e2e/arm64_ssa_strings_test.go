...
func TestArm64SSAStringChurnHoldsTheHeapFlat(t *testing.T) {
	src := `import "std/i32";
...
`
	ssaOut, ssaCode := runArm64SSAProgram(t, src, t.TempDir())
	if ssaCode >= 8 {
		t.Errorf("the SSA build's heap grew %d KiB over 10,000 string rounds (101 = the length check failed):\n%s", ssaCode, ssaOut)
	}
	// The flat build, through the same CLI without the backend flag.
	...
	flatBin := filepath.Join(dir, "flat.bin")
	if o, err := exec.Command(buildFernForArm64SSA(t), "-target", "arm64-linux", "-o", flatBin, srcPath).CombinedOutput(); err != nil {
		t.Fatalf("flat compile: %v\n%s", err, o)
	}
	flatCmd := runArm64Bin(qemu, flatBin)
	flatOut, _ := flatCmd.CombinedOutput()
	if flatCode := flatCmd.ProcessState.ExitCode(); flatCode != ssaCode {
		t.Errorf("heap growth in KiB: ssa=%d flat=%d\n%s%s", ssaCode, flatCode, ssaOut, flatOut)
	}
}
