...
func odCases(t *testing.T) []invocation {
	...
	cases := []invocation{
		...
		{name: "size letter H float", args: []string{"-t", "fH", allBytes}},
		{name: "size letter B float", args: []string{"-t", "fB", allBytes}},
		{name: "half and bfloat side by side", args: []string{"-t", "fH", "-t", "fB", "-t", "x2", allBytes}},
		{name: "half floats over arbitrary bytes", args: []string{"-t", "fH", random}},
		{name: "bfloats over arbitrary bytes", args: []string{"-t", "fB", random}},
		{name: "half floats big-endian", args: []string{"--endian=big", "-t", "fH", "-t", "fB", allBytes}},
		{name: "a half beside a float", args: []string{"-t", "fH", "-t", "f4", allBytes}},
		...
		{name: "an offset with a trailing point is decimal", args: []string{short, "+4."}},
		{name: "an offset with a point before its b", args: []string{short, "+4.b"}},
		{name: "an offset with a point inside it is a file", args: []string{short, "+4.0"}},
		{name: "a hex offset with a trailing point is not hex", args: []string{short, "+0x4."}},
		{name: "an offset too large to hold", args: []string{short, "+99999999999999999999."}},
		{name: "an octal offset too large to hold", args: []string{short, "+7777777777777777777777777"}},
