...
func condCases(t *testing.T) []invocation {
	...
	return []invocation{
		...
		// A terminal on each of 0, 1 and 2, which is what the pty fields
		// are for: `-t` is the only predicate whose answer a pipe fixes,
		// and every case above it answers no by construction.
		{name: "t zero is a terminal", args: []string{"-t", "0"}, ttyIn: true},
		{name: "t one is a terminal", args: []string{"-t", "1"}, ttyOut: true},
		{name: "t two is a terminal", args: []string{"-t", "2"}, ttyErr: true},
		{name: "t zero with a terminal on one only", args: []string{"-t", "0"}, ttyOut: true},
		{name: "t one with a terminal on zero only", args: []string{"-t", "1"}, ttyIn: true},
		{name: "t two with a terminal on zero only", args: []string{"-t", "2"}, ttyIn: true},
		{name: "t zero and t one", args: []string{"-t", "0", "-a", "-t", "1"}, ttyIn: true, ttyOut: true},
		{name: "t zero or t one", args: []string{"-t", "0", "-o", "-t", "1"}, ttyIn: true},
		{name: "all three are terminals", args: []string{"-t", "0", "-a", "-t", "1", "-a", "-t", "2"}, ttyIn: true, ttyOut: true, ttyErr: true},
		{name: "t three is not a terminal with one on zero", args: []string{"-t", "3"}, ttyIn: true},
