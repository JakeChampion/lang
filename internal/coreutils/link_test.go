...
func linkCases(t *testing.T) []invocation {
	return []invocation{
		...
		{name: "creates a hard link", args: []string{"src", "dst"}, tree: linkTree},
		{name: "a second link to the same file", args: []string{"src", "another"}, tree: linkTree},
		// link(2) does not follow the final symlink of either operand, so
		// the new name is a second link to the LINK, not to its target.
		{name: "a symlink is linked to itself", args: []string{"sym", "dst"}, tree: linkTree},
		{name: "a dangling symlink links fine", args: []string{"dangling", "dst"}, tree: linkTree},

		// The error paths, each with its own errno text.
		{name: "the new name is taken", args: []string{"src", "taken"}, tree: linkTree},
		{name: "the target does not exist", args: []string{"nope", "dst"}, tree: linkTree},
		{name: "the target is a directory", args: []string{"adir", "dst"}, tree: linkTree},
		{name: "the new name is a directory", args: []string{"src", "adir"}, tree: linkTree},
		{name: "the new name has no parent", args: []string{"src", "nodir/dst"}, tree: linkTree},
		{name: "the new name is under a file", args: []string{"src", "src/dst"}, tree: linkTree},
		{name: "linking a file to itself", args: []string{"src", "src"}, tree: linkTree},
		{name: "empty operands", args: []string{"", ""}, tree: linkTree},
		{name: "empty target", args: []string{"", "dst"}, tree: linkTree},
		{name: "empty new name", args: []string{"src", ""}, tree: linkTree},
		...
		{name: "names with a space", args: []string{"no such src", "no such dst"}, tree: linkTree},
		{name: "names with an apostrophe", args: []string{"a'b", "c'd"}, tree: linkTree},
		{name: "names with a newline", args: []string{"a\nb", "c\nd"}, tree: linkTree},
		{name: "names that are not valid UTF-8", args: []string{"\xff\xfe", "\xfd\xfc"}, tree: linkTree},

		// `-` is an ordinary operand here; `--` ends the options.
		{name: "lone dash target", args: []string{"-", "dst"}, tree: linkTree},
		{name: "dashdash then operands", args: []string{"--", "src", "dst"}, tree: linkTree},
		{name: "dashdash alone", args: []string{"--"}},
		{name: "dashdash with one operand", args: []string{"--", "src"}, tree: linkTree},
		...
		{name: "stdout closed on success", args: []string{"src", "dst"}, tree: linkTree, stdout: stdoutClosed},
