...
// binary-size epic, #4109/#4112). The thresholds are coarse — well clear of the
// measured ratios (loop ~89%, arith ~77%, large mixed programs ~84%) — so the
// test catches a real emit-quality regression (e.g. losing the result-into-home
// coalescing or the call-clobber-aware caller-save) without flaking on small
// deltas. See docs/SSA-REGALLOC-PLAN.md for the phase results.
func TestCodeSizeSmallerThanStackMachine(t *testing.T) {
	...
	// The SSA path should be at least ~10% smaller here (measured ~84%).
	ssaB, smB := textSizes(t, genMixedProgram(100))
	ratio := float64(ssaB) / float64(smB)
	if ratio > 0.90 {
		t.Errorf("large program: SSA .text=%d is %.0f%% of stack machine=%d, want <=90%%", ssaB, ratio*100, smB)
	}
