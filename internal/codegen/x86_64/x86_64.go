...
func (g *generator) emitAllocRuntime() {
	const heapBytes = 17179869184 // 0x400000000 (16 GiB) per region — sized so a cmd/fern-built self-host compiler can bootstrap-compile the WHOLE self-host source in one process. The stage-2 self-compile live set crossed the exit-137 alloc trap at 2.5 GiB, 3.5 GiB, then filled 99.4% of the 3.875 GiB arena (3944 MB measured at the #4355 arr-of-arr slice, whose additions tipped it over); the u32 mmap-length form capped further bumps under 4 GiB (an interim 0xFF000000/3.984 GiB bump was superseded by this), so the length now loads via `movabs rsi` (64-bit imm). Raised again 8 GiB → 16 GiB when the x86 stage-2 self-compile refilled the 8 GiB arena — a single mmc2 self-compile of the source + any ~56-line compiler-source addition deterministically tripped exit-137 (confirmed with `-parallel 1`, so it is the arena wall, not host-OOM from the fixpoint's parallel cases). This mirrors the arm64 backend, which crossed the same 8 GiB trap earlier and is already at 16 GiB (measured need ~8.2 GiB at tip); x86 now matches. The compiler's live set grows with every compiler-source addition, so the roadmap's goal-1 IR widening needs this headroom to land. MAP_NORESERVE (0x4022) makes the wider reservation free — untouched pages cost nothing and the map is exempt from Linux's overcommit refusal (a 16 GiB map would otherwise fail AT STARTUP on a 16 GB host with no swap). Actual peak RSS stays ~8 GiB, safely under a 16 GB CI host; the exit-137 bounds check in the allocator remains the real out-of-memory guard. Matches the self-host emitters' own `heap_size` (asm.fern / asm_ir.fern = 17179869184) — keeping the native (stage-0 mmc) and self-host x86 heaps in lockstep. Lazy MAP_ANONYMOUS so it costs nothing until touched. This native heap is an mmap region addressed via REGISTER (not a static `.bss` block), so it has no RIP-relative / imm32 displacement ceiling — heap_base + heap_size may exceed 2 GiB. The heap-END is built `movabs rcx, heapBytes` + `add rcx, rax` rather than a signed-disp32 `lea [base + heapBytes]` (which caps at 0x7FFFFFFF). The self-host emitters reach their `.bss` heap via 64-bit-absolute `movabs $__fern_heap` with __fern_heap emitted LAST in .bss so its > 2 GiB base never truncates a PC32 relocation.
	g.line("")
