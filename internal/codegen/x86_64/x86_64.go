...
func (g *generator) emitAllocRuntime() {
	const heapBytes = 3758096384 // 0xE0000000 (3.5 GiB) per region — sized so a cmd/fern-built self-host compiler can bootstrap-compile the WHOLE self-host source in one process. As the IR subset widened (enum-array Expr[]/Stmt[] aliasing), the stage-2 self-compile live set crossed 2.5 GiB (exit-137 alloc trap); 3.5 GiB restores headroom and matches the arm64 backend (at its 0x10000000-base + 3.5 GiB = 0xF0000000 < 4 GiB ceiling). Matches the self-host emitters' own `heap_size` (asm.fern / asm_ir.fern = 3758096384) — keeping the native (stage-0 mmc) and self-host heaps in lockstep. Lazy MAP_ANONYMOUS so it costs nothing until touched. This native heap is an mmap region addressed via REGISTER (not a static `.bss` block), so it has no RIP-relative / imm32 displacement ceiling — heap_base + heap_size may exceed 2 GiB. heapBytes is the `mov esi, heapBytes` mmap length (u32 zero-extend, fits) and the heap-END is built `mov ecx, heapBytes` (u32) + `add rcx, rax` rather than a signed-disp32 `lea [base + heapBytes]` (which caps at 0x7FFFFFFF). The self-host emitters reach their `.bss` heap via 64-bit-absolute `movabs $__fern_heap` with __fern_heap emitted LAST in .bss so its > 2 GiB base never truncates a PC32 relocation.
	g.line("")
