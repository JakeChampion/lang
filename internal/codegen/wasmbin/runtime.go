		...
		for _, op := range fn.Ops {
			...
			case ir.OpCallDirect, ir.OpRcInc, ir.OpRcDec, ir.OpRcIsUnique:
				...
				case "__fern_reader_read_chunk":
					...
					// returned as Ok(chunk) string → rc1. The
					// scratch, an EOF'd buffer and (preview 2)
					// the host's raw list go back through __free.
					// A failing read classifies its errno, so the
					// IoError constructor comes with it.
					needs.add("__fern_alloc") // rc1 calls it
					needs.add("__fern_alloc_rc1")
					needs.add("__free")
				...
				case "strbuf_reset", "strbuf_append", "strbuf_take":
					...
					needs.add(op.Str)
				case "buf_new", "buf_push", "buf_push_range", "buf_push_mapped", "buf_push_filtered", "buf_push_byte",
...
// pokes it calls — are scanned into the set the way a user function's are,
// and a helper may need another, so the walk repeats until no Fern helper
// is left in the set. The program is copied rather than appended to in
// place. The returned set names the injected functions.
func injectFernHelpers(prog *ir.Program, needs *runtimeNeeds, opts EmitOptions) (*ir.Program, map[string]bool, error) {
	...
	for {
		...
		for _, name := range pending {
			...
			sub := scanRuntimeHelpers(&ir.Program{Funcs: []*ir.Func{fn}}, opts)
	...
	if len(funcs) == 0 {
		return prog, injected, nil
	}
	var kept runtimeNeeds
	for _, n := range needs.order {
		if !injected[n] {
			kept.add(n)
		}
	}
	*needs = kept
	p := *prog
	p.Funcs = append(append([]*ir.Func(nil), prog.Funcs...), funcs...)
...
var runtimeHelperSpecs = map[string]runtimeHelperSpec{
	...
	"__fern_scan_set": {
		// (data, len, from, set) → i32 index of the first byte whose set
		// entry is nonzero, or len.
		params:  []byte{encode.ValtypeI32, encode.ValtypeI32, encode.ValtypeI32, encode.ValtypeI32},
		results: []byte{encode.ValtypeI32},
	...
	"__load_u8": {
		// (addr) → i32 — i32.load8_u wrapper; the byte the fernrt
		// helpers scan with.
		params:  []byte{encode.ValtypeI32},
		results: []byte{encode.ValtypeI32},
		body:    buildLoadU8Body,
	},
	"__store_i32": {
	...
	"buf_new": {
		// (cap) → handle. Reserves the control block and its buffer.
		params:  []byte{encode.ValtypeI32},
		results: []byte{encode.ValtypeI32},
		body:    buildBufNewBody,
	},
	"__fern_buf_reserve": {
		// (h, need) → (). Internal: the growth the three pushes share.
		params:  []byte{encode.ValtypeI32, encode.ValtypeI32},
		results: nil,
		body:    buildBufReserveBody,
	},
	"buf_push": {
		// (h, data, len) → (). Every byte of the string onto the tail.
		params:  []byte{encode.ValtypeI32, encode.ValtypeI32, encode.ValtypeI32},
		results: nil,
		body:    buildBufPushBody,
	},
	"buf_push_range": {
		// (h, data, len, lo, hi) → (). A byte range, no intermediate.
		params: []byte{encode.ValtypeI32, encode.ValtypeI32, encode.ValtypeI32,
...
//
//	[H +  0] data     — the buffer's string DATA pointer, 0 when unarmed
//	[H +  4] len      — bytes accumulated so far
...
func bufReserveCall(body []byte, reserve, hLocal, needLocal uint32) []byte {
	...
	body = inst.InstLocalGet(body, needLocal)
	return inst.InstCall(body, reserve)
}

// bufDst pushes `mem[h+0] + mem[h+4]`, the address the next byte lands at.
// Read AFTER a reserve: a grow moves the buffer.
func bufDst(body []byte, hLocal uint32) []byte {
	body = inst.InstLocalGet(body, hLocal)
	body = memory.InstI32Load(body, 2, 0)
	body = inst.InstLocalGet(body, hLocal)
...
// buildBufNewBody assembles wasm bytes for buf_new.
//
// Signature: (cap i32) → (handle i32). Locals: $h (1).
func buildBufNewBody(idxs map[string]uint32) []byte {
	allocRc1 := idxs["__fern_alloc_rc1"]
	var body []byte
	// cap = max(cap, 16) — a floor, so the doubling has something to double.
	body = inst.InstLocalGet(body, 0)
	body = inst.InstI32Const(body, 16)
	body = numeric.InstI32LtU(body)
	body = inst.InstIfStart(body, inst.BlocktypeEmpty)
...
// buildBufReserveBody assembles wasm bytes for __fern_buf_reserve.
//
// Signature: (h, need i32) → (). Locals: $ncap (2), $np (3), $old (4).
//
// Internal: no Fern builtin reaches it, the three pushes do.
func buildBufReserveBody(idxs map[string]uint32) []byte {
	allocRc1 := idxs["__fern_alloc_rc1"]
	boxFree := idxs["__fern_box_free"]
	var body []byte
	body = inst.InstLocalGet(body, 1)
	body = inst.InstLocalGet(body, 0)
	...
	body = numeric.InstI32GtU(body)
	body = inst.InstIfStart(body, inst.BlocktypeEmpty)
	// ncap = cap, or the reserve when a take left the builder unarmed,
	// or 64; then double until it covers need.
	body = inst.InstLocalGet(body, 0)
	body = memory.InstI32Load(body, 2, 8)
	body = inst.InstLocalSet(body, 2)
	body = inst.InstLocalGet(body, 2)
	...
	body = inst.InstCall(body, allocRc1)
	body = inst.InstLocalSet(body, 3)
	// Carry the live bytes across. A zero-length copy from the unarmed
	// null pointer moves nothing and is in bounds.
	body = inst.InstLocalGet(body, 3)
	body = inst.InstLocalGet(body, 0)
	body = memory.InstI32Load(body, 2, 0)
	body = inst.InstLocalGet(body, 0)
...
// buildBufPushBody assembles wasm bytes for buf_push.
//
// Signature: (h, data, len i32) → (). Locals: $sl (3), $need (4), $dst (5),
// $i (6).
//
// A heap-form argument is contiguous in linear memory and moves with one
// memory.copy; an inline/SSO one lives IN its two words with no address to
// copy from, so its bytes come out through __fern_str_byte.
func buildBufPushBody(idxs map[string]uint32) []byte {
	strLen := idxs["__fern_str_len"]
	strByte := idxs["__fern_str_byte"]
	...
	body = numeric.InstI32Add(body)
	body = inst.InstLocalSet(body, 4)
	body = bufReserveCall(body, reserve, 0, 4)
	body = bufDst(body, 0)
	body = inst.InstLocalSet(body, 5)
	body = bufCopyBytes(body, strByte, 1, 2, 3, 5, 6, noByteOffset)
	body = inst.InstLocalGet(body, 0)
	body = inst.InstLocalGet(body, 4)
	body = memory.InstI32Store(body, 2, 4)
...
// noByteOffset says a copy starts at the string's first byte, so the heap
// branch needs no address adjustment and the inline branch no index bias.
const noByteOffset = ^uint32(0)

// bufCopyBytes emits "copy `n` bytes of the string in (dataLocal, rawLenLocal),
// starting at byte `offLocal` (or 0 when offLocal is noByteOffset), to the
// address in dstLocal". iLocal is scratch for the inline branch's loop.
func bufCopyBytes(body []byte, strByte, dataLocal, rawLenLocal, nLocal, dstLocal, iLocal, offLocal uint32) []byte {
	// Heap form is (raw_len & 0x80000000) == 0.
	body = inst.InstLocalGet(body, rawLenLocal)
	body = inst.InstI32Const(body, int32(-0x80000000))
	body = numeric.InstI32And(body)
	body = numeric.InstI32Eqz(body)
	body = inst.InstIfStart(body, inst.BlocktypeEmpty)
...
//
// Signature: (h, data, len, lo, hi i32) → (). Locals: $n (5), $need (6),
// $dst (7), $i (8).
//
// An empty or inverted range is a no-op; the bounds themselves are the
// caller's, as with slice_unchecked.
func buildBufPushRangeBody(idxs map[string]uint32) []byte {
	strByte := idxs["__fern_str_byte"]
	reserve := idxs["__fern_buf_reserve"]
	var body []byte
	body = inst.InstLocalGet(body, 4)
	...
	body = numeric.InstI32Add(body)
	body = inst.InstLocalSet(body, 6)
	body = bufReserveCall(body, reserve, 0, 6)
	body = bufDst(body, 0)
	body = inst.InstLocalSet(body, 7)
	body = bufCopyBytes(body, strByte, 1, 2, 5, 7, 8, 3)
	body = inst.InstLocalGet(body, 0)
	body = inst.InstLocalGet(body, 6)
	body = memory.InstI32Store(body, 2, 4)
	return inst.PutFunctionBody(nil, inst.PutLocalsOneGroup(nil, 4, encode.ValtypeI32), body)
...
func buildBufPushByteBody(idxs map[string]uint32) []byte {
	reserve := idxs["__fern_buf_reserve"]
	var body []byte
	body = inst.InstLocalGet(body, 0)
	body = memory.InstI32Load(body, 2, 4)
...
// buildBufLenBody assembles wasm bytes for buf_len.
//
// Signature: (h i32) → (i32).
func buildBufLenBody(_ map[string]uint32) []byte {
	var body []byte
	body = inst.InstLocalGet(body, 0)
	body = memory.InstI32Load(body, 2, 4)
	return inst.PutFunctionBody(nil, inst.PutLocalsEmpty(nil), body)
...
//
// Signature: (h i32) → (). Locals: $data (1).
func buildBufFreeBody(idxs map[string]uint32) []byte {
	boxFree := idxs["__fern_box_free"]
	var body []byte
	body = inst.InstLocalGet(body, 0)
	body = numeric.InstI32Eqz(body)
...
// first byte at or after `from` whose entry in `set` is nonzero, or the
// length. Scalar. `set` is a u8[] data pointer with its length at -4; a byte
// past that length is not in the set.
//
// Locals after the four params: $n (4), $i (5), $m (6, the set length),
// $b (7, the byte).
func buildScanSetBody(idxs map[string]uint32) []byte {
	strLen := idxs["__fern_str_len"]
	strByte := idxs["__fern_str_byte"]
	const (
		pData = 0
	...
	// Every byte goes through __fern_str_byte, which reads an inline string's
	// two words and a heap string's buffer alike.
	body = inst.InstBlockStart(body, inst.BlocktypeEmpty)
	body = inst.InstLoopStart(body, inst.BlocktypeEmpty)
	body = inst.InstLocalGet(body, lI)
	body = inst.InstLocalGet(body, lN)
	...
	// if $b < $m and mem[$set + $b] != 0: return $i
	...
	body = inst.InstLocalGet(body, lN)
	locals := inst.PutLocalsOneGroup(nil, 4, encode.ValtypeI32) // $n, $i, $m, $b
	return inst.PutFunctionBody(nil, locals, body)
}
...
// buildLoadU8Body — (addr) → i32. Single i32.load8_u at offset 0.
func buildLoadU8Body(_ map[string]uint32) []byte {
	var body []byte
	body = inst.InstLocalGet(body, 0)
	body = memory.InstI32Load8U(body, 0, 0)
	return inst.PutFunctionBody(nil, inst.PutLocalsEmpty(nil), body)
...
// arrHeaderBytes is the array header that precedes a Fern array's data
// pointer on wasm32: pad at data-16, capacity at data-12, refcount at
// data-8, length at data-4, elements at data. Wider element strides use
// max(16, stride) so element 0 stays stride-aligned (see
// buildArrDecBody); every helper in this package builds a stride <= 16
// array.
const arrHeaderBytes = 16

// arrRcOwned is the refcount a freshly built array carries: the caller
// owns the one reference and drops it at scope exit. arrRcStatic is the
// "static, never touch" sentinel __fern_rc_inc / __fern_rc_dec /
// __fern_arr_dec all short-circuit on — the right header for a
// process-lifetime singleton handed to every caller by pointer, where
// no one reference can be the last.
const (
	arrRcOwned  int32 = 1
	arrRcStatic int32 = -0x80000000
)

// emitArrHeaderStore appends the cap / rc / len stores for the array
// whose data pointer is already in dataLocal. pushCap and pushLen push
// the capacity and the element count; they differ only for a
// grow-on-demand buffer that ends up longer than it is full.
func emitArrHeaderStore(body []byte, dataLocal uint32, rc int32, pushCap, pushLen func([]byte) []byte) []byte {
	// mem[data-12] = cap
	body = inst.InstLocalGet(body, dataLocal)
	body = inst.InstI32Const(body, 12)
	body = numeric.InstI32Sub(body)
	...
	body = inst.InstI32Const(body, 8)
	body = numeric.InstI32Sub(body)
	body = inst.InstI32Const(body, rc)
	body = memory.InstI32Store(body, 2, 0)
	// mem[data-4] = len
	body = inst.InstLocalGet(body, dataLocal)
	body = inst.InstI32Const(body, 4)
...
// emitArrHeaderAlloc appends
//
//	data = __fern_alloc(arrHeaderBytes + bytes) + arrHeaderBytes
//	mem[data-12] = count ; cap
//	mem[data-8]  = rc
//	mem[data-4]  = count ; len
//
// leaving the data pointer in dataLocal. pushCount pushes the element
// count, pushBytes the payload byte length.
//
// Every array a runtime helper hands back to Fern code goes through
// here. A bare 4-byte length prefix is not enough: __fern_rc_dec and
// __fern_arr_dec read the refcount at data-8, so the caller's
// scope-exit dec would land on whatever precedes the allocation.
func emitArrHeaderAlloc(body []byte, alloc, dataLocal uint32, rc int32, pushCount, pushBytes func([]byte) []byte) []byte {
	...
	body = inst.InstI32Const(body, arrHeaderBytes)
	body = numeric.InstI32Add(body)
	body = inst.InstLocalSet(body, dataLocal)
	return emitArrHeaderStore(body, dataLocal, rc, pushCount, pushCount)
}

// buildAllocU8Body — (n) → i32. Allocates a zero-filled u8[] of
// length n with the canonical header (see emitArrHeaderAlloc) and
...
// buildArgsBody — () → i32. Builds a string[] of all argv
// entries and returns its data pointer. Each entry is a 2-word
// (data, len) pair in heap form: data points into argv_buf, len
// is the strlen (top bit clear).
//
// The array carries the canonical cap / rc / len header, with the rc
// word set to the static sentinel rather than 1: the cache below hands
// the same pointer to every caller, so no single scope-exit dec can be
// the last one and an owned header would free the array out from under
// the cache on the first `for a in args()` that ends.
//
...
func buildArgsBody(helperIdxs map[string]uint32) []byte {
	...
	body = inst.InstReturn(body)
	body = inst.InstEnd(body)
	// Build: $result = a fresh argc-element string[] (16-byte header).
	body = inst.InstI32Const(body, argsCountAddr)
	body = memory.InstI32Load(body, 2, 0)
	body = inst.InstLocalSet(body, 0) // $argc
	pushArgc := func(b []byte) []byte { return inst.InstLocalGet(b, 0) }
	body = emitArrHeaderAlloc(body, alloc, 4, arrRcStatic, pushArgc, func(b []byte) []byte {
		b = inst.InstLocalGet(b, 0)
	...
	body = inst.InstLoopStart(body, inst.BlocktypeEmpty)
	{
		...
		body = inst.InstLocalSet(body, 9) // $clen
		body = inst.InstLocalSet(body, 8) // $cdata
...
// builds the string[] of argv (same immortal header) from the
...
//
// Locals: 0=$rb, 1=$argc, 2=$result, 3=$list, 4=$i, 5=$el, 6=$cdata,
// 7=$clen.
func buildArgsBodyP2(helperIdxs map[string]uint32) []byte {
	...
	body = memory.InstI32Load(body, 2, 0)
	body = inst.InstLocalSet(body, 1)
	// $result = a fresh argc-element string[] (16-byte header).
	pushArgc := func(b []byte) []byte { return inst.InstLocalGet(b, 1) }
	body = emitArrHeaderAlloc(body, alloc, 2, arrRcStatic, pushArgc, func(b []byte) []byte {
		b = inst.InstLocalGet(b, 1)
		b = inst.InstI32Const(b, 8)
		return numeric.InstI32Mul(b)
	})
	...
	body = inst.InstLoopStart(body, inst.BlocktypeEmpty)
	{
		...
		body = memory.InstI32Load(body, 2, 4)
		body = inst.InstCall(body, strCopy)
		body = inst.InstLocalSet(body, 7) // $clen
		body = inst.InstLocalSet(body, 6) // $cdata
