...
// an entry in the flat arm64 backend's table (arm64.AbortMsg), which is where
// the text and the exit status come from: a program's abort output must not
// depend on which arm64 emitter built it (#5538). The sanitizer diagnostics
// are absent because their detectors are.
//
...
// A check site is then a single branch instead of the three instructions it used to
// open-code — so a program with more than about forty
// bounds checks is smaller for having the diagnostics than it was without
// them.
...
// error instead of a silent branch to whichever kind sorts next to it.
const (
...
// abortKindFor picks the tail an inlined index check branches to: the same
// two messages the __arr_idx / __slice_idx helpers give, since the inline form
// is those helpers open-coded at the call site.
func abortKindFor(slice bool) string {
	if slice {
...
func emitAbortTails(w func(string, ...any), emitted string) {
	...
	for _, i := range used {
		k := abortKinds[i]
		text, code := arm64.AbortMsg(k.msgSym)
		w("%s:", k.tail)
		w("\tmov x0, #2") // stderr
		w("\tadrp x1, %s", k.msgSym)
		w("\tadd x1, x1, #:lo12:%s", k.msgSym)
		w("\tmov x2, #%d", len(text))
		w("\tmov x8, #64") // write
		w("\tsvc #0")
		w("\tmov x0, #%d", code)
	...
	for _, i := range used {
		k := abortKinds[i]
		text, _ := arm64.AbortMsg(k.msgSym)
...
func emitAllocPres(w func(string, ...any)) {
	...
	w("\tstp x8, x9, [sp, #80]")
	w("\tstp x10, x11, [sp, #96]")
	w("\tstp x12, x13, [sp, #112]")
	w("\tstp x14, x15, [sp, #128]")
	w("\tmrs x17, nzcv")
	w("\tmov x0, x16")
	...
	w("\tldp x8, x9, [sp, #80]")
	w("\tldp x10, x11, [sp, #96]")
	w("\tldp x12, x13, [sp, #112]")
	w("\tldp x14, x15, [sp, #128]")
	w("\tldp x29, x30, [sp], #144")
	w("\tret")
}

// allocPresLines is the inline splice: total block size in x16 on entry, the
// 16-aligned block base in x16 on exit; every other register survives, x0..x15
// and x17 and the flags restored by the trampoline, and x30 stacked here
// because the site may be a leaf helper that keeps its return address there.
...
// emitAllocBlock writes an __alloc of sizeReg+extra bytes through the
// trampoline and leaves the block base in baseReg. Only x16 and baseReg
// change; the block spans its whole size class, which is what lets a string
// grow in place and __free take it back at its length.
func emitAllocBlock(w func(string, ...any), baseReg, sizeReg string, extra int) {
	w("\tadd x16, %s, #%d", sizeReg, extra)
	emitAllocPresCall(w)
	w("\tmov %s, x16", baseReg)
}
...
// emitHandleTtyHelper writes a terminal question's handle form: the fd out of
// the box and a branch into the free builtin's helper, which leaves every
// argument after the receiver where that helper wants it (#9363). Same shape
// as emitHandleIsattyHelper, and the same fd offset.
...
// emitEmptyString allocates a zero-length single-word rc string into `dst`
// (rc=1, len=0, one NUL byte). Used to supply a valid (empty) path to
// __fern_io_error for write/close errors, which carry no path. Clobbers x7
// and x16.
...
func emitReaderReadChunkHelper(w func(string, ...any)) {
	...
	w("\tand x4, x4, #-16")
	w("\tadd x5, x10, #9")
	emitFreelistClass(w, "rrc_req", "x5", "x6", "x1", "x2", "x7", "x13", "x14", "x15", ".Lssa_rrc_req_none")
	w(".Lssa_rrc_req_none:")
	w("\tadd x6, x4, x5")
	w("\tstr x6, [x3]")
	emitHeapGuardCall(w)
	...
	// A short read (a pipe hands back at most 64 KiB) keeps only the class
	// the bytes it produced fill; the rest of the buffer goes back to the
	// cursor.
	w("\tadd x5, x0, #9") // header + bytes + NUL
	emitFreelistClass(w, "rrc_got", "x5", "x6", "x1", "x2", "x7", "x13", "x14", "x15", ".Lssa_rrc_got_none")
	w(".Lssa_rrc_got_none:")
	w("\tadd x1, x4, x5") // base + that class's extent, never above the cursor
...
func emitStrAppendHelper(w func(string, ...any)) {
	...
	w("\tcmp x0, #0x10000")
	w("\tb.lo .Lssa_strapp_copy")
	w("\tldur w2, [x0, #-8]") // rc; a static sentinel has its top bit set
	w("\tcmp w2, #1")
	w("\tb.ne .Lssa_strapp_copy")
	w("\tldur w3, [x0, #-4]") // la
	w("\tldur w4, [x1, #-4]") // lb
	...
	w("\tstur w5, [x0, #-4]") // len = total
	w("\tstp x29, x30, [sp, #-32]!")
	w("\tstr x0, [sp, #16]")
	w("\tadd x7, x0, x3") // dst = a + la
	emitBcopyCall(w, "x7", "x1", "x4")
	w("\tldr x0, [sp, #16]")
	w("\tldp x29, x30, [sp], #32")
	w("\tret")
	w(".Lssa_strapp_copy:")
...
func emitStrDecHelper(w func(string, ...any)) {
	...
	w("\tcmp w1, #1")
	w("\tb.lt .Lssa_strdec_ret") // already dropped
	w("\tb.eq .Lssa_strdec_free")
	w("\tsub w1, w1, #1")
	w("\tstur w1, [x0, #-8]")
	w(".Lssa_strdec_ret:")
	w("\tret")
...
func emitArgsHelper(w func(string, ...any)) {
	...
	w("\tadrp x4, %s", heapPtrSym)
	w("\tadd x4, x4, #:lo12:%s", heapPtrSym) // x4 = &cursor
	w("\tldr x5, [x4]")
	w("\tadd x5, x5, #15")
...
func emitReadFileHelper(w func(string, ...any)) {
	...
	// and give the outgrown block back.
	w("\tlsl x24, x22, #1")
	w("\tcmp x24, #4096")
	w("\tb.hs .Lssa_rf_grow")
	w("\tmov x24, #4096")
	...
	w("\tb .Lssa_rf_regrow")
	w(".Lssa_rf_regrown:")
	w("\tsub x0, x21, #8") // the outgrown block
	w("\tadd x1, x22, #9") // at the size it was requested
	w("\tmov x21, x5")
	w("\tmov x22, x24")
	w("\tbl %s", fnLabel("__free"))
	w(".Lssa_rf_read:")
...
func emitReadFileBytesHelper(w func(string, ...any)) {
	...
	w("\tb .Lssa_rfb_regrow")
	w(".Lssa_rfb_regrown:")
	w("\tsub x0, x21, #16") // the outgrown block
	w("\tadd x1, x22, #16") // at the size __alloc_u8 requested
	w("\tmov x21, x5")
	w("\tmov x22, x24")
	w("\tbl %s", fnLabel("__free"))
	w(".Lssa_rfb_read:")
...
func emitBufPushFilteredHelper(w func(string, ...any)) {
	...
	w("\tadd x5, x5, #1")
	w("\tcmp x5, x12")
	w("\tb.lo .Lssa_buffilt_full")
	w("\tb .Lssa_buffilt_len")
	w(".Lssa_buffilt_short:")
	w("\tldrb w6, [x11, x5]")
	w("\tcmp w6, w4")
	w("\tb.hs .Lssa_buffilt_keep")
	w("\tldrb w8, [x2, x6]")
	w("\tcbnz w8, .Lssa_buffilt_next")
	w(".Lssa_buffilt_keep:")
	w("\tstrb w6, [x9, x7]")
	w("\tadd x7, x7, #1")
	w(".Lssa_buffilt_next:")
...
// and __str_concat use), with no small-string inline optimisation, so this is an
// __alloc plus an __ssa_bcopy. bs is the input u8[] data pointer;
...
func inlineArrIdxLines(in x86.Inst, fr frameLayout, numAlloc int, seed string) ([]string, bool) {
	...
	if form.checked {
		...
		out = append(out,
			...
			fmt.Sprintf("b %s", abortKindFor(form.slice)),
			ok+":",
		)
	}
