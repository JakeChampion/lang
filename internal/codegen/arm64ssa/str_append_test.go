...
// "ab"+"cd" is a 4-byte string in the 16-byte class (13 bytes requested, since
// a block carries the rc word, the length word and the trailing NUL), so the
// class holds 7 bytes of string: three more fit exactly and a fourth does not.
...
func TestArmStrAppendMovesOncePerSizeClass(t *testing.T) {
	...
	// A class of c bytes holds c-9 bytes of string — the rc word, the length
	// word and the trailing NUL (strBlockBytes): 127 moves up through the
