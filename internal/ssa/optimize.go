...
// FoldBranches, CSE, DCE — in that order, iterating until the
// function's printed form stops changing. Returns the iteration
// count (useful in tests and for the upcoming `lang dump-ssa
// -opt-iters` flag).
//
// The order matters:
...
//  4. CSE dedups pure expressions left over after Simplify
//     unified equivalent arg lists.
//  5. DCE reclaims everything orphaned by the four previous
//     passes.
//
...
// removes a phi arg, the phi becomes trivial in the next
// iteration's Simplify pass — once we add trivial-phi
// elimination).
//
