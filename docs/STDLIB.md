...
The context-sensitive rules are not applied, so a word-final `Σ`
lowercases to `σ` where Unicode says `ς`. Not locale-aware, by design.
The tables are regenerated by `cmd/unicodegen`; the full-mapping data
is the one table with no build-time oracle, since Go's stdlib has
