...
are not, by design.
The tables are regenerated by `cmd/unicodegen`. Most have no build-time
oracle in Go's stdlib, so their source data is derived offline and
checked in, from two different oracles:

- the full case mappings (Go has simple mappings only) and the
  normalization data (Go has neither canonical decompositions nor
  combining classes) come from CPython's `unicodedata`, via
  `gen_normdata.py` → `normdata.txt`;
- `Grapheme_Cluster_Break`, `Extended_Pictographic` and `Word_Break`
  are exposed by neither, so they come from `uniseg` (PyPI), via
  `gen_gcbdata.py` → `gcbdata.txt` and `gen_wbdata.py` → `wbdata.txt`.

Both oracles are **regeneration-time** dependencies only; the module
itself has none. The segmentation data tracks a newer UCD than the case
and normalization data, which is internally consistent because
boundaries never consult those tables — and because that data is *not*
frozen by a stability policy, each generated table records the version
it came from. Hangul composes and decomposes arithmetically rather than
by table.
...
- One-shots: `md5_bytes(s: string): u8[]` / `md5_hex(s): string`, and the
  same pair for `sha1`, `sha224`, `sha256`, `sha384`, `sha512`;
