...
scripts/coreutils-bench.d/<util>.sh
                    that utility's workloads, sourced with its name in $1
```
...
5. Add the utility's workloads as `scripts/coreutils-bench.d/<util>.sh` and
   record its first numbers in the sub-issue. If it is slower than GNU, that
   is the next task, not a footnote.
