...
(1450 upper + 1433 lower mappings, Unicode 15.0.0) generated by
`cmd/unicodegen` from the Go stdlib's `unicode` package — re-runnable on
a toolchain upgrade, deterministic output. Binary search over sorted
key arrays; `to_upper`/`to_lower` build the tables once per call then
loop (no const-array caching yet), so a whole-string map is O(table +
n) — fine at CLI scale. **Caveats**: simple mappings only (multi-code-
point expansions like `ß`→`SS` are left unchanged, matching Go), and
not locale-aware. Covered by `examples/tests/unicode_test.fern` and
