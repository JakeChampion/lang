...
`eq_ignore_case(a, b)`), character classes (`is_letter`, `is_digit`,
`is_alnum`, `is_whitespace`, `is_upper`, `is_lower`), normalization
(`nfc` / `nfd` / `is_nfc` / `is_nfd` / `eq_canonical`) and UAX #29
segmentation (`graphemes` / `grapheme_count` / `reverse_graphemes`,
`word_segments` / `words` / `word_count`).
...
predicates for every code point in 0..MaxRune. **Caveats**: the
