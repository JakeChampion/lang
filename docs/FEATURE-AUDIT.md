...
| `temp_dir(prefix)` | | | | | | ⬜ | |
| `subprocess(...)` | | | | | | ⬜ | |
| `sleep_ms` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | all backends ([#2843](https://github.com/JakeChampion/lang/issues/2843)); self-host **IR path** lowers it on x86-64/arm64 (wasm routes via the AST path); native wasm landed in [#7947](https://github.com/JakeChampion/lang/issues/7947) — preview-1 `poll_oneoff` on one monotonic-clock subscription, preview-2 subscribe-duration + block + drop (`internal/codegen/wasmbin/wasi_sleep_test.go`) |
...
| `udp_send` | ❌ | ✅ | ✅ | ✅ | ✅ | ⚠️ | one datagram to a dotted-quad IPv4 literal; the interpreter has no builtin. Native since #10278 (`TestX86_64UdpSend`, `TestArm64UdpSend`); a bad host answers -3 natively and in the self-host, -28 on wasm |
| `map_new` + Map methods | | | | | | ⬜ | |
...
composite-key use (`Opt[(i32, i32)]`) is left out of the pass entirely, erased
as before, since a clone beside the generic original mis-dispatches on the AST
lowering. The enum's methods and derives clone per instantiation as a struct's
do, and a method with a type parameter of its own folds into a free generic
first (2026-09-22, `docs/rc-log/`). Coverage: `TestSelfHostGenericEnum{IRX86_64,WasmIR}` —
