...
| Lambdas (anonymous `function(…)` + arrow `(x: T): R => e`) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | typed params required, return type optional (#2701). An expression body is the returned value; a BRACED body is the lambda's own body, spliced in statement for statement, so a trailing value written without a `;` is returned and a body that yields nothing is void (#2673). Unlike NAMED functions (which must annotate — E070), a lambda may omit its return type and have it inferred |
