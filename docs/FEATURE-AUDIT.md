...
| `std/u32` | ✅ | ✅ | ✅ | ✅ | ✅ | ⚠️ | max — `audit_std_path_numeric`; self-host unsigned min/max via the x86-64 IR path (`TestSelfHostNumericMethodsIRX86_64`); u32 unsigned compares now correct on all IR backends — x86/arm64 always were (zero-extended), wasm `i32.*_u` fixed ([#2917](https://github.com/JakeChampion/lang/issues/2917); `TestSelfHostU32CmpWasmIR`) |
