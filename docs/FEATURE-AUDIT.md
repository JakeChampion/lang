...
| `std/sort` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | `prop_sort_i32` — ordering + permutation (histogram) + idempotence laws (native, all four backends); self-host via the IR path (x86-64 + wasm): `TestSelfHostSortIR` — i32 ascending/descending stable bottom-up merge sorts (every non-consuming `sort_*` is now O(n log n); only the `fip` `sort_i32_inplace_*` variants remain insertion-sort, since `fip` cannot allocate merge scratch), the byte-lexicographic `string_cmp` three-way comparator, and the `string[]` sorts built on it (`.with` element rewrite on an aliased CoW scratch buffer, indexed scalar + string-byte reads, nested merge `while` loops), oracle-checked. **Generic comparator sort** `sort_by[T](arr, cmp: (T,T)=>i32): T[]` + `is_sorted_by[T]` added (the closure-arg form the module header long deferred — now that fn-typed args lower, incl. in loop conditions): any element type / custom ordering, no `Ord` bound. Coverage: `TestNativeSortBy{,Module,Arm64}` + `TestSelfHostSortByIR{X86_64,Wasm}` |
