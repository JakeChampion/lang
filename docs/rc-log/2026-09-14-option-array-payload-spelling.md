...
Three rows in `TestSelfHostWithCowIR{X86_64,Arm64,Wasm}`:
`option-local-payload-append`, `option-local-payload-with`,
`option-local-nested-payload-append`; each bails on the parent commit. Also
green: the Option / match / payload / reclaim families of
`internal/e2eselfhost`, the whole-compiler emit-all fixpoint, the
