...
| Self-host checker code SEQUENCE (`internal/e2eselfhost/self_host_checker_code_sequence_test.go`) | The ORDER the self-host emits diagnostics in and HOW MANY TIMES it emits each code — the one thing every other checker gate reduces away before comparing. Row-based: each pins one program's codes joined in emission order, duplicates kept. Pins CURRENT behaviour, not a claim it is right; the codes differential remains the gate for WHICH codes are correct. Earned its place on its first run by exposing a nested-lambda E049 gap (#7363), and has since caught three more in the E060/E062 pair — two downcasts silently accepted and a struct literal's `...base` visited after its fields | Every program outside its rows, and every difference the code sequence cannot express. Two diagnostics with the SAME code swapping places is invisible here — only their messages move, and no gate compares message ORDER — so a row probing traversal order needs two DIFFERENT codes in the two slots to see anything |
...
- **The ORDER of the self-host checker's diagnostics, and how many times it
  ...
  nothing else does — so the residual gap is everything outside its rows, plus
  one case it cannot express by construction: **two diagnostics with the same
  code swapping places.** The sequence is unchanged and only the messages move,
  which no gate compares. A row probing traversal order therefore needs two
  DIFFERENT codes in the two slots, which is why the struct-literal row pins an
  E062 in the base and an E060 in a field.
