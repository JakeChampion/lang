...
| Differential (`internal/e2e/diff_oracle_test.go`) | The NATIVE compiler's backends agree with interp on exit codes, over 2048 fernsmith-generated programs — 43% of which now carry a lambda and 6% an escaping closure | Everything about memory — see below. The SELF-HOST compiler: no fernsmith harness routes the corpus through it, so the closure-dispatch cluster (#5001/#5007/#5009/#5026), which lived on the self-host IR path, is still not mechanically reachable. Also anything the generator cannot emit — a shape absent from `gtype` is untested no matter how many seeds run |
