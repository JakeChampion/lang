...
- **`FERN_IR_VERIFY=1`** — reach for this one FIRST when the self-host compiles
  ...
  Off by default; observation only, so the emitted code is byte-identical
  either way and a diagnosis made under it is about the same compiler that
  failed without it. **Do not set it on the wasm-HOSTED compiler**: the gate's
  stack pass corrupts the WAT that compiler emits (#6994), a latent fault the
  gate surfaces rather than causes, and the reason it is not on by default
  yet.
