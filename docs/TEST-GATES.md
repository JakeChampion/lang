...
| Driver size gate (`driver-sizes` job, `scripts/ci-check-driver-sizes`) | That the linked size of the 15 self-host drivers has not run away from `.github/selfhost-driver-sizes.txt`. Two thresholds, because one number cannot be both: 5% warns, 15% fails. An ordinary day moves these binaries ~3% (whole-compiler) to ~7% (front-end slices), so a 5% gate would red on ordinary work; the event it exists for was +16.1% on the tracked artifact. It also fails when a baselined driver went unmeasured, since the report is assembled from four jobs' artifacts and a lost one would narrow the comparison silently | WHY a binary grew — it compares totals, and cannot attribute. Anything between two runs that both stay inside 15%: sustained drift is caught only cumulatively, when nobody has re-baselined |
