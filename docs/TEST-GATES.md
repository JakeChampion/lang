...
| Self-host SSA backend (`internal/e2eselfhost/self_host_ssa_backend_test.go`, `TestSelfHostSSABackend*`) | That the self-host register path (`docs/SELFHOST-SSA-BACKEND.md`), the only emitter on the native ISAs, agrees with the NATIVE compiler: each program is compiled by both for every target the host can run output for (its own ISA natively, the other under qemu when present), run, and compared on stdout AND exit status. The programs are the shapes the register path once got wrong or emits specially (loop-carried phis, immediates, the box layouts, the OS floor through the flat-op arm, dyn dispatch). Also that `-backend ssa` is byte-identical to the default, that `-backend flat` is refused on the native ISAs and names wasm's one emitter, and the emitted-shape pins (frame sized by spills, the five-instruction loop, inline rc primitives). Builds the CLI for the host (arm64-darwin Mach-O on Apple Silicon, arm64-linux ELF on arm64 Linux, x86-64 ELF on x86-64 Linux) | Every program outside its list — the corpus is the fixture legs below, where a function the lift cannot take is a compile FAILURE naming the op (there is no stack machine to decline onto, so coverage needs no tally). And the emitted code's size or speed, which the ssa-log entries carry |
...
**The sweep sees every emitter there is.** The native ISAs have one, the
register path, and its op table for the OS floor (`emit_stack_op`, run
between a push of the operands and a pop of the result) is reached by every
corpus program that uses one of those ops, so a change to an arm moves the
sweep like any other codegen change.
