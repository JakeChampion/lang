...
| Formatter corpus properties (`internal/printer/corpus_test.go`, plus `TestSelfHostFmtCorpusParityX86_64`) | That `-fmt` does not rewrite the program. Three properties over all 427 `.fern` files under examples/ + internal/stdlib rather than a fixture list: the formatted output still TYPE-CHECKS (where the input did), it re-parses to the same AST modulo position (the structural one — the only property that sees data loss whose output still compiles, e.g. `Box[i64]` dropping to `Box[i32]`), and formatting is idempotent. The self-host leg adds byte-parity with native over the same corpus, against an allowlist that is exact in both directions | Comment PLACEMENT, which no property here pins — a comment moved onto the wrong declaration still type-checks, re-parses identically (they are a side table) and is stable. Only byte-parity with native catches that, and only while native is right |
