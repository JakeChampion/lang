...
| Self-host SEMANTIC differential (`internal/e2e/diff_oracle_selfhost_test.go`, `TestDifferential_SelfHostSemanticX86_64`, `FERN_SELFHOST_DIFF=1`) | The same leg with `FERN_SEM_IR=1`: the self-host compiler's semantic lowering (`semsource` / `ssaunits` / `ssarc`, `docs/SELFHOST-SEMANTIC-SOURCE.md`) agrees with interp on exit codes over the same corpus, and every seed that compiles produces EVERY declaration, read from the compiler's own tally (`requireSemWhole`), so the path cannot quietly refuse its way back to the AST lowering. The arm64 and wasm legs hold the same. Per PR as `diff-selfhost-x86_64` and nightly in four shards over the moving window; its known-divergences file is exact and empty | A seed that does not compile at all is a skipped gap, so a refusal the checker reaches first is not counted. And the other two backends, which the two rows below cover — this leg is x86-64 only, and the semantic lowering emits target-specific runtime of its own |
