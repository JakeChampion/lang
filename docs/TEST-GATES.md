...
| Assembler vocabulary tables (`internal/native/x86tbl`, `internal/native/arm64tbl`; `cmd/x86tblgen` / `cmd/arm64tblgen` staleness + every-row tests; `internal/e2eselfhost` `TestSelfHostArm64TableRowsMatchNative`) | That the two assemblers of each ISA agree about WHICH mnemonics exist. One Go table per ISA is the vocabulary: the Go assembler dispatches by looking a mnemonic up in it, and the self-host's predicates, allow-list and base-word lookups are generated from it between marker comments in the `.fern` — the staleness test regenerates and diffs, the markers test keeps that from passing vacuously, and the every-row test assembles each row's probe through the Go assembler. On arm64 the same probes then go through the self-host assembler and the word must match native: that is the gate on a mnemonic the allow-list claims and the dispatch does not reach (the #6060 shape). The scrape it replaced read `mnem == "…"` literals out of the Fern source and could not see a family dispatched by pattern; its `^b\.?[a-z]{2}$` condition-alias exclusion also matched `brk`, which sat unported inside it | Encoder LOGIC: the tables carry vocabulary, operand shape and, for the FP / carry / widening-multiply / negated-logical / conditional-select families, the base word — how a family turns operands into the rest of the word is still hand-written on each side, and the form differentials next door are what compare that. The x86-64 side has no table-row self-host differential yet; its coverage gate still scrapes `x86_gas_emit` |
