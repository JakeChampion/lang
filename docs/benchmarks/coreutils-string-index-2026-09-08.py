...
"""Checked string-index inlining experiment for #8278; run only in a disposable Linux /bench.

Place native -O binaries at /bench/sort, /bench/sort-ssa-fixed (before),
and /bench/sort-ssa-index (after) and the uutils
...
See ../COREUTILS-STRING-INDEX-2026-09-08.md for build and measurement details.
"""
...
for binary in [root / 'sort', root / 'sort-ssa-fixed', root / 'sort-ssa-index', Path('/usr/bin/sort'), root / 'uutils/coreutils']:
...
impls = {'fern-default': [str(root / 'sort')], 'ssa-before': [str(root / 'sort-ssa-fixed')], 'ssa-after': [str(root / 'sort-ssa-index')], 'gnu': ['/usr/bin/sort'], 'uutils': [uu, 'sort']}
