...
"""Checked ssa-branches inlining experiment for #8278; run only in a disposable Linux /bench.

Place native -O binaries at /bench/sort, /bench/sort-ssa-index (before),
and /bench/sort-ssa-branches (after) and the uutils
...
See ../COREUTILS-SSA-BRANCHES-2026-09-08.md for build and measurement details.
"""
...
for binary in [root / 'sort', root / 'sort-ssa-index', root / 'sort-ssa-branches', Path('/usr/bin/sort'), root / 'uutils/coreutils']:
...
impls = {'fern-default': [str(root / 'sort')], 'ssa-before': [str(root / 'sort-ssa-index')], 'ssa-after': [str(root / 'sort-ssa-branches')], 'gnu': ['/usr/bin/sort'], 'uutils': [uu, 'sort']}
...
for utility, args, source in cases:
    commands = {name: prefix + args + [str(source)] for name, prefix in impls.items()}
    oracle = None
