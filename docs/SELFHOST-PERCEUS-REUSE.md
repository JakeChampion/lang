...
- The runtime uniqueness guard **at the reuse site**: the token shape —
  `__fern_rc_is_unique(d)` selects a token, `__fern_alloc_reuse(token, nfields)`
  hands back d's block or a fresh one, and a shared donor degrades to a fresh
  construction instead of corrupting — has landed for three of the five emitter
  families: **`emit_self_overwrite_reuse`** (#4350 slice 1; carried fields copied
  + rc-inc'd, d's reference dec'd on the fresh arm), **`emit_cross_struct_reuse`**
  (#4350 slice 2; old pointer fields released on the reuse arm, shape word copied
  on the fresh arm), and **`emit_cross_tuple_reuse`** (#4350 slice 3; simpler —
  leak-mode ⇒ no old-element release, no shape word ⇒ no shape copy, just the
  donor dec; `nfields = arity - 1` since alloc_reuse reserves a struct shape
  slot a tuple box lacks). **The remaining two families (`enum-donor`/`inarm`)
  are still unguarded** — their soundness rests on the syntactic escape walk
  alone until their slices land (#4350).
...
- **Memory corruption** is the dominant risk. Mitigations: (a) the runtime
  ...
  slices 1–3 that covers the self-overwrite, cross-struct, and cross-tuple
  families; enum-donor and inarm remain static-walk-only until their #4350
  slices land; (b) each slice ships a
