...
- [~] **SH-020 — Create `examples/self_host/util.fern`** (sibling, import-friendly)
  ...
  `disasm`, `vm`, `constfold`, `printer` (dead copy deleted), and `ssa_x86`/`ssa_arm64`/`ssa_wasm` are converted (7 of 9 done).
  Remaining: convert the last 2 `i32_to_string` copies (`wasm`, `asmcore` — footprints 58/74, so their staging-edit sweep is the large part) and the
