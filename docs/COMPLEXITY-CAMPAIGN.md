...
The stdlib is a separate, much smaller front (ceiling 68, excess 778) with no
emit-hash oracle behind it — its gate is the ordinary test suites. It was
measured once the self-host patterns were established, and **this campaign's
technique does not fit it.** Two reasons, both from the numbers:

- **The ceiling is not hand-editable.** All of it is `unicode.fern`, which says
  `GENERATED by cmd/unicodegen … Do NOT edit by hand`: 87 excess and the whole
  68 ceiling live there, in four functions. Lowering that number means changing
  the generator, which is a different task with a different justification.
- **The rest is flat.** The 691 hand-written excess is spread over 88 functions
  with a top of 48 — for scale, `lower_call_method` alone opened at 472. Eight
  functions at 30 or more hold 31% of it; 67 functions between 11 and 29 hold
  the rest. Probing the eight largest found exactly one with an extractable
  shape (`__rx_atom`, three always-returning guards, worth perhaps 15).

Splitting a 15-complexity function into two 8s is the numbered-helper mistake at
small scale: the metric improves and the reader gains nothing. Leave the stdlib
alone unless a single function grows into a real monster.
