...
| Closures / lambdas | ✅ `(x: T) => e`, escaping + capturing | **102** — 4 anonymous `function(…)` exprs and 98 nested named fns (4 astwalk visitors, 19 `wasm_ir` helper-gate predicates behind `any_op`, 22 in `checker`'s collectors, 16 in `parser` — the mentions, fn-value-call, moves-handle, deep-defer-scan, elb-guard and hl families — and 25 in `irlower` — the cap-family visitors, body_binds_lambda's, the closure-array credit's two (#4354), the five visit/descend pairs of the Perceus escape-scanner trio, and the env-box lift's nine accumulator-spine wrappers, #6993); the visitors mostly capture, the gate predicates mostly do not — plus **6** arrow lambdas (3 no-op statement visitors, 3 in `constfold`'s assert probe) | pinned |
