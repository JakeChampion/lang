...
| Closures / lambdas | ✅ `(x: T) => e`, escaping + capturing | **59** — 4 anonymous `function(…)` exprs and 55 nested named fns (4 astwalk visitors, 17 `wasm_ir` helper-gate predicates behind `any_op`, 22 in `checker`'s collectors, 12 in `parser` — the mentions, fn-value-call, moves-handle, deep-defer-scan and elb-guard families); 31 of the 59 capture, the gate predicates mostly do not — plus **6** arrow lambdas (3 no-op statement visitors, 3 in `constfold`'s assert probe) | pinned |
...
| `astwalk` call sites (walkers on the shared spine) | — | **117** across 12 modules — `parser.fern` joins with the mentions, fn-value-call, moves-handle, deep-defer-scan and elb-guard families (#6993) | floor |
