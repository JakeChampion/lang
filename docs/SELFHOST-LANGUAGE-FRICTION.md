...
| Closures / lambdas | ✅ `(x: T) => e`, escaping + capturing | **45** — 4 anonymous `function(…)` exprs and 41 nested named fns (4 astwalk visitors, 17 `wasm_ir` helper-gate predicates behind `any_op`, 20 in `checker`'s collectors); 22 of the 45 capture, the gate predicates mostly do not — plus **6** arrow lambdas (3 no-op statement visitors, 3 in `constfold`'s assert probe) | pinned |
