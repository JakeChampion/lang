...
| i32/i64 → string | **2-digit table, exact-size buffer** | Lemire multiply-shift, 2-digit table | **DONE** (the `/ 100` is a multiply-high on x86-64) |
| string → int | Byte-at-a-time accumulate | SWAR 8-digit chunks | GAP — **blocked on the same raw-load question as wyhash (#6200)** |
| Division by constant | Magic-number reciprocal on x86-64, i32 and i64 | Magic-number reciprocal in the compiler | **DONE on x86-64**; arm64 still divides |
...
10. ~~Magic-number constant division in the compiler~~ — done on x86-64;
    arm64 wants a hardware measurement first (`docs/CODEGEN-QUALITY-AUDIT-2026-09.md` §6.2).
...
The SOTA entry is Lemire's multiply-shift: replace the division with a
fixed-point reciprocal multiply. On x86-64 the compiler now does that for any
division by a constant, `/ 100` included, so the stdlib gets it without writing
one by hand. arm64 still divides (the magic-number-division row further down).
