...
def main():
    ...
    for index, workers in enumerate((1, 2, 2, 1)):
        directory = args.output / f"trial-{index}-workers-{workers}"
        print(f"Starting trial {index}: {workers} workers, total CPU budget {cpus}", flush=True)
        before = resource.getrusage(resource.RUSAGE_CHILDREN)
        begin = time.perf_counter()
        if workers == 1:
            directory.mkdir()
            # Preserve the production go-test launch environment. In particular,
            # test2json as a parent changes inherited INT/QUIT dispositions.
            subprocess.run([
                "gotestsum", "--format", "pkgname-and-test-fails", "--jsonfile",
                str(directory.resolve() / "baseline.jsonl"), "--", "-run", pattern,
                "-count=1", "-timeout=25m", f"-parallel={cpus}", "./internal/e2e",
            ], cwd=root, env=env, check=True)
        else:
            subprocess.run([
                str(args.runner.resolve()), "-binary", str(args.binary.resolve()),
                "-output", str(directory.resolve()), "-workers", str(workers),
                "-cpus", str(cpus), "-timeout", "25m", "-run", pattern,
            ], cwd=root / "internal/e2e", env=env, check=True)
        wall = time.perf_counter() - begin
        after = resource.getrusage(resource.RUSAGE_CHILDREN)
        actual_parents, outcomes = (verify_events(directory / "baseline.jsonl", parents)
                                   if workers == 1 else collect_outcomes(directory))
