...
Each `.lang` file is a self-contained program. The header comment
explains what it does, what features it showcases, and the
expected output (or curl shape, for the HTTP examples).
...
|---|---|---|
| `hello.lang` | wasm | The minimum: `function main(): i32 { print(...) }` |
| `factorial.lang` | wasm | Recursion, `i32` arithmetic |
| `fizzbuzz.lang` | wasm | Loops, byte-level `putchar` |
| `array.lang` | wasm | Fixed-size array literal, indexing |
| `sum_for.lang` | wasm | `for` loop, accumulator |
| `wc.lang` | wasm | argv, stdin / `Reader` API, file I/O via `open_reader`, `defer`, `Result + match` for fallible operations |
