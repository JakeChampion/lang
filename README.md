...
IR optimisation → native (ARM64 `.s`, Linux ELF or Mach-O via `-target
arm64-darwin`; x86-64 Linux ELF) or WASM (preview-2 component) emitter. Every
backend consumes the same `ir.Program`, so a new language feature usually needs
only `Lower` + the IR; codegen picks it up for free.
...
- Types: sized integers (`i8`/`i16`/`i32`/`i64`, `u8`/`u16`/`u32`/`u64`, with
  `isize`/`usize` aliases), `f32`/`f64` floats, `boolean`, `void`, `string`,
  arrays (`i32[]`), nominal structs, function types (`(T, U) => V`). Integer
  literals are polymorphic in expected-type context (`var x: i64 = 1` needs no
  cast); mixing widths requires an explicit `as`.
- Operators: `+ - * / %`, `== != < > <= >=`, `&& || !`, bitwise
...
| `TailCallOptimize` | Wraps the body in a loop and rewrites `OpCallDirect <self> ; OpReturn` to a parameter rebind plus `OpBr`, giving O(1) stack depth on self-tail recursion. Wired into every backend (`arm64.go`, `x86_64.go`, `wasmbin/build.go`). |
...
Concrete payoff — `function f(): i32 { var x: i32 = 7; var y: i32 = x + 3;
return y * 2 + x; }` lowers to twelve IR ops and collapses to a single
`const.i32 27 ; return` after the pipeline.
